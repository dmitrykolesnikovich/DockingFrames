 ...
 */
@ColorCodes({"stack.tab.border.out.selected", "stack.tab.border.center.selected", "stack.tab.border.out.focused",
  "stack.tab.border.center.focused", "stack.tab.border.out", "stack.tab.border.center", "stack.tab.border.out.disabled",
  "stack.tab.border.center.disabled", "stack.tab.border",

  "stack.tab.background.top.selected", "stack.tab.background.bottom.selected", "stack.tab.background.top.focused",
  "stack.tab.background.bottom.focused", "stack.tab.background.top", "stack.tab.background.bottom", "stack.tab.background.top.disabled",
  "stack.tab.background.bottom.disabled", "stack.tab.background",

  "stack.tab.foreground.selected", "stack.tab.foreground.focused", "stack.tab.foreground.disabled", "stack.tab.foreground"})
  ...
  public FlatTab(FlatTabPane pane, Dockable dockable) {
    ...
    foreground = new FlatTabColor("stack.tab.foreground", dockable) {
      ...
      protected void changed(Color oldColor, Color newColor) {
        if (!isSelected() && isEnabled()) setForeground(newColor);
      }
    };
...
//        setOpaque( false );
    ...
    setBorder(new Border() {
      public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
        ...
        if (isSelected()) {
          if (out == null) out = borderSelectedOut.value();
          if (center == null) center = borderSelectedCenter.value();
        }
        if (out == null) out = borderOut.value();
        if (center == null) center = borderCenter.value();

        if (out == null || center == null) {
          Color background = border.value();
          if (background == null) background = FlatTab.this.background.value();

          if (background == null) background = getBackground();

          if (out == null) out = background;
  ...
  public void paintBackground(Graphics g) {
    if (getTransparency() != Transparency.TRANSPARENT) {
      ...
      if (isSelected()) {
        if (top == null) top = backgroundSelectedTop.value();
        if (bottom == null) bottom = backgroundSelectedBottom.value();
      }
      if (top == null) top = backgroundTop.value();
      if (bottom == null) bottom = backgroundBottom.value();

      if (top == null || bottom == null) {
        Color background = FlatTab.this.background.value();
        if (background == null) background = getBackground();

        if (bottom == null) bottom = background;
