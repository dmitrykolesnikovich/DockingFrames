...
public class ArchGradientPainter extends BaseTabComponent {
  ...
  protected Arch arch(int width, int height) {
    if (arch == null || arch.getWidth() != width || arch.getHeight() != height) arch = new Arch(width, height);
    return arch;
  }
  ...
  public boolean contains(int x, int y) {
    ...
    if (isSelected()) {
      ...
      if (left.contains(x, y)) return true;

      Polygon right = rightSide(0, 0, w, h);
      if (right.contains(x, y)) return true;

      Rectangle leftBox = left.getBounds();
      Rectangle rightBox = right.getBounds();

      if (getOrientation().isHorizontal()) {
        if (leftBox.x + leftBox.width > x) return false;

        if (rightBox.x < x) return false;
      }
      else {
        if (leftBox.y + leftBox.height > y) return false;

        if (rightBox.y < y) return false;
      }
  ...
  private void paintSelected(Graphics g, Color top, Color bottom) {
    ...
    if (firstTab) {
      ...
      else if (orientation.isVertical() && getY() <= 1) left.translate(0, -1);
    }
