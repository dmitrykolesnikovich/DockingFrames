...
@ColorCodes({"stack.tab.border", "stack.tab.border.selected", "stack.tab.border.selected.focused", "stack.tab.border.selected.focuslost",
  ...
  "stack.tab.top", "stack.tab.top.selected", "stack.tab.top.selected.focused", "stack.tab.top.selected.focuslost", "stack.tab.top.disabled",
...
public abstract class BaseTabComponent extends ConfiguredBackgroundPanel implements TabComponent, EclipseTabStateInfo {
  ...
  public BaseTabComponent(EclipseTabPane pane, Dockable dockable, String colorPostfix) {
    ...
    colors = new TabColor[]{colorStackTabBorder, colorStackTabBorderSelected, colorStackTabBorderSelectedFocused,
      colorStackTabBorderSelectedFocusLost, colorStackTabBorderDisabled, colorStackTabTop, colorStackTabTopSelected,
      colorStackTabTopSelectedFocused, colorStackTabTopSelectedFocusLost, colorStackTabTopDisabled, colorStackTabBottom,
      colorStackTabBottomSelected, colorStackTabBottomSelectedFocused, colorStackTabBottomSelectedFocusLost, colorStackTabBottomDisabled,
      colorStackTabText, colorStackTabTextSelected, colorStackTabTextSelectedFocused, colorStackTabTextSelectedFocusLost,
      colorStackTabTextDisabled, colorStackBorder};
  ...
  public void bind() {
    if (buttons != null) {
      actions = new EclipseDockActionSource(pane.getTheme(), dockable.getGlobalActionOffers(), this, true);
  ...
  public void unbind() {
    bound = false;
    if (buttons != null) buttons.set(null);
  ...
  protected boolean isPreviousTabSelected() {
    if (previousTabSelectedSet) return previousTabSelected;
  ...
  protected boolean isNextTabSelected() {
    if (nextTabSelectedSet) return nextTabSelected;

    EclipseTabPane pane = getPane();

    int self = getTabIndex();
    if (self >= pane.getVisibleTabCount()) return false;
