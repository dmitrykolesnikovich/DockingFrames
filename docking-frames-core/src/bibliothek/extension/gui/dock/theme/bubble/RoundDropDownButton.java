...
@ColorCodes({"action.dropdown", "action.dropdown.enabled", "action.dropdown.selected", "action.dropdown.selected.enabled",
  "action.dropdown.mouse.enabled", "action.dropdown.mouse.selected.enabled", "action.dropdown.pressed.enabled",
  "action.dropdown.pressed.selected.enabled",

  "action.dropdown.focus", "action.dropdown.enabled.focus", "action.dropdown.selected.focus", "action.dropdown.selected.enabled.focus",
  "action.dropdown.mouse.enabled.focus", "action.dropdown.mouse.selected.enabled.focus", "action.dropdown.pressed.enabled.focus",
  "action.dropdown.pressed.selected.enabled.focus",

  "action.dropdown.line", "action.dropdown.line.enabled", "action.dropdown.line.selected", "action.dropdown.line.selected.enabled",
  "action.dropdown.line.mouse.enabled", "action.dropdown.line.mouse.selected.enabled", "action.dropdown.line.pressed.enabled",
  "action.dropdown.line.pressed.selected.enabled",

  "action.dropdown.text", "action.dropdown.text.enabled", "action.dropdown.text.selected", "action.dropdown.text.selected.enabled",
  "action.dropdown.text.mouse.enabled", "action.dropdown.text.mouse.selected.enabled", "action.dropdown.text.pressed.enabled",
...
public class RoundDropDownButton extends JComponent implements RoundButtonConnectable {
  ...
  public RoundDropDownButton(BasicDropDownButtonHandler handler, Dockable dockable, DockAction action) {
    ...
    colors = new RoundActionColor[]{new RoundActionColor("action.dropdown", dockable, action, Color.WHITE),
  ...
  public void updateUI() {
    ...
    if (handler != null) handler.updateUI();
  }
  ...
  public Dimension getPreferredSize() {
    ...
    if (model.getOrientation().isHorizontal()) {
      return new Dimension((int)(0.5 * contentSize.height + contentSize.width + 1 + 1.5 * dropIcon.getIconWidth()),
                           (int)(1.5 * contentSize.height));
    }
    else {
      return new Dimension((int)(1.5 * contentSize.width),
                           (int)(0.5 * contentSize.width + contentSize.height + 1 + 1.5 * dropIcon.getIconHeight()));
    }
  }
  ...
  private void doPaintForeground(Graphics g) {
    Icon drop = dropIcon;
    if (!isEnabled()) {
      if (disabledDropIcon == null) disabledDropIcon = handler.getDisabledDropDownIcon();
    ...
    if (model.getOrientation().isHorizontal()) {
      ...
      if (drop != null) drop.paintIcon(this, g, (int)(x + w - 1.25 * dropIconWidth), y + (h - dropIconHeight) / 2);
      ...
      if (drop != null) drop.paintIcon(this, g, x + (w - dropIconWidth) / 2, (int)(y + h - 1.25 * dropIconHeight));
  ...
  public void updateColors() {
    ...
    if (selected) postfix = ".selected";

    if (enabled) postfix += ".enabled";
    ...
    for (RoundActionColor color : colors) {
      if (background.equals(color.getId())) animation.putColor("background", color.value());
      if (mouse.equals(color.getId())) animation.putColor("mouse", color.value());
      if (focus.equals(color.getId())) animation.putColor("focus", color.value());
      if (text.equals(color.getId())) animation.putColor("text", color.value());
    }
  }
