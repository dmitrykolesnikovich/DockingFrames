...
public class MergedPreferenceModel extends AbstractPreferenceModel {
  ...
  public MergedPreferenceModel(PathCombiner combiner, DockController controller) {
    super(controller);
    if (combiner == null) throw new IllegalArgumentException("combiner must not be null");
  ...
  public void insert(int index, PreferenceModel model, Path path) {
    if (this == model) throw new IllegalArgumentException("model must not be this");

    for (Model check : models) {
      if (check.model == model) throw new IllegalArgumentException("can't add a model twice");

      if (check.path.equals(path)) throw new IllegalArgumentException("there is already a model with the path " + path);
    }
    ...
    if (hasListeners()) model.addPreferenceModelListener(listener);
  ...
  public void remove(int index) {
    ...
    if (hasListeners()) model.model.removePreferenceModelListener(listener);
  ...
  public int indexOf(Path path) {
    int i = 0;
    for (Model check : models) {
      if (check.path.equals(path)) return i;
  ...
  public String getLabel(int index) {
    ...
    if (local == null) throw new ArrayIndexOutOfBoundsException(index);
  ...
  protected int indexAt(PreferenceModel model, int index) {
    for (Model check : models) {
      if (check.model == model) return index;
