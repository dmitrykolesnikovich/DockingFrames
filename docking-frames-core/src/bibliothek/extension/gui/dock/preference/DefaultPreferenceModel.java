...
public class DefaultPreferenceModel extends AbstractPreferenceModel {
  ...
  public void insert(int index, Preference<?> preference) {
    if (preference == null) throw new IllegalArgumentException("preference must not be null");

    if (index < 0 || index > entries.size()) throw new ArrayIndexOutOfBoundsException(index);
  ...
  public int indexOf(Preference<?> preference) {
    for (int i = 0, n = entries.size(); i < n; i++) {
      if (entries.get(i).getPreference() == preference) return i;
    }
