...
public class DefaultChoice<V> implements Choice {
  ...
  public DefaultChoice(DockController controller) {
    if (controller != null) {
      ExtensionName<ChoiceExtension> name =
        new ExtensionName<ChoiceExtension>(ChoiceExtension.CHOICE_EXTENSION, ChoiceExtension.class, ChoiceExtension.CHOICE_PARAMETER, this);
  ...
  public int indexOfIdentifier(String id) {
    if (id == null) {
      for (int i = 0, n = list.size(); i < n; i++) {
        if (list.get(i).getEntryId() == null) return i;
      }
    }
    else {
      for (int i = 0, n = list.size(); i < n; i++) {
        if (id.equals(list.get(i).getEntryId())) return i;
      }
    }
  ...
  public int indexOfValue(V value) {
    ...
    for (int i = 0, n = list.size(); i < n; i++) {
      if (equals(list.get(i).getEntryValue(), value)) return i;
    }
  ...
  public String getDefaultChoice() {
    if (defaultChoice == null) {
      if (isNullEntryAllowed()) return null;

      if (list.size() > 0) return list.get(0).getEntryId();
