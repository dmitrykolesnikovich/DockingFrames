...
public class PreferenceTreeModel extends AbstractPreferenceModel implements TreeModel {
  ...
  private void fireNodeAdded(TreeNode parent, TreeNode child) {
    ...
    if (listeners.length > 0) {
      TreeModelEvent event = new TreeModelEvent(this, parent.getTreePath(), new int[]{parent.indexOf(child)}, new Object[]{child});
  ...
  private void fireNodeChanged(TreeNode node) {
    ...
    if (listeners.length > 0) {
      ...
      if (parent == null) {
        event = new TreeModelEvent(this, node.getTreePath());
      }
      else {
        event = new TreeModelEvent(parent, parent.getTreePath(), new int[]{parent.indexOf(node)}, new Object[]{node});
      }
  ...
  private void fireNodeRemoved(TreeNode parent, int[] indices, Object[] children) {
    ...
    if (listeners.length > 0) {
      ...
      TreeModelEvent event = new TreeModelEvent(this, parent.getTreePath(), indices, children);
  ...
  private class TreeNode implements Node {
    ...
    public TreeNode getNode(Path path, int segment) {
      if (segment == path.getSegmentCount()) return this;
