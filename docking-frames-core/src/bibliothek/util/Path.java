...
public final class Path {
  ...
  public Path(String path) {
    ...
    for (int i = 0, n = path.length(); i <= n; i++) {
      ...
      if (escape) {
        ...
        if (c == '.') {
          if (lastDot + 1 == i) throw new IllegalArgumentException("not a path: empty segment");
          ...
          if (!Character.isJavaIdentifierStart(c)) throw new IllegalArgumentException("not a valid start of a segment: '" + c + "'");
        }
        else {
          if (!Character.isJavaIdentifierPart(c)) throw new IllegalArgumentException("not a valid character of a segment: '" + c + "'");
        }
      }
    }
  ...
  public Path subPath(int offset, int length) {
    if (length < 1) throw new IllegalArgumentException("length must be at least 1: " + length);
