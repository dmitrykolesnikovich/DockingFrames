...
public class SplitDockStation extends SecureContainer implements Dockable, DockStation {
  ...
  public static final PropertyKey<SplitDividerStrategy> DIVIDER_STRATEGY =
    new PropertyKey<SplitDividerStrategy>("SplitDockStation divider strategy", new DynamicPropertyFactory<SplitDividerStrategy>() {
  ...
  private PropertyValue<SplitLayoutManager> layoutManager = new PropertyValue<SplitLayoutManager>(LAYOUT_MANAGER) {
    ...
    protected void valueChanged(SplitLayoutManager oldValue, SplitLayoutManager newValue) {
      if (oldValue != null) oldValue.uninstall(SplitDockStation.this);

      if (newValue != null) newValue.install(SplitDockStation.this);
    }
  };
  /**
  ...
  public void setFullScreenAction(ListeningDockAction fullScreenAction) {
    if (this.fullScreenAction != null) throw new IllegalStateException("The fullScreenAction can only be set once");
  ...
  public void setDockParent(DockStation station) {
    if (this.parent != null) this.parent.removeDockStationListener(visibleListener);

    parent = station;

    if (station != null) station.addDockStationListener(visibleListener);
  ...
  public void setController(DockController controller) {
    ...
    if (this.controller != controller) {
      ...
      if (fullScreenAction != null) fullScreenAction.setController(controller);
  ...
  public void setSideSnapSize(float sideSnapSize) {
    if (sideSnapSize < 0) throw new IllegalArgumentException("sideSnapSize must not be less than 0");
  ...
  public void setBorderSideSnapSize(int borderSideSnapSize) {
    if (borderSideSnapSize < 0) throw new IllegalArgumentException("borderSideSnapeSize must not be less than 0");
  ...
  public void setDividerSize(int dividerSize) {
    if (dividerSize < 0) throw new IllegalArgumentException("dividerSize must not be less than 0");
  ...
  public void bind(DockTitle title) {
    if (titles.contains(title)) throw new IllegalArgumentException("Title is already bound");
    titles.add(title);
    for (DockableListener listener : dockableListeners.toArray(new DockableListener[dockableListeners.size()])) {
  ...
  public void unbind(DockTitle title) {
    if (!titles.contains(title)) throw new IllegalArgumentException("Title is unknown");
    titles.remove(title);
    for (DockableListener listener : dockableListeners.toArray(new DockableListener[dockableListeners.size()])) {
  ...
  public DockActionSource getIndirectActionOffers(Dockable dockable) {
    if (fullScreenAction == null) return null;

    DockStation parent = dockable.getDockParent();
    if (parent == null) return null;

    if (parent instanceof SplitDockStation) return null;
  ...
  public Dockable getFrontDockable() {
    if (isFullScreen()) return getFullScreen();

    if (frontDockable == null && dockables.size() > 0) frontDockable = dockables.get(0).getDockable();
  ...
  public void setFrontDockable(Dockable dockable) {
    ...
    if (isFullScreen() && dockable != null) setFullScreen(dockable);
   ...
   */
  protected boolean dropAside(SplitNode neighbor,
                              PutInfo.Put put,
                              Dockable dockable,
                              Leaf leaf,
                              double divider,
  ...
  private void replace(Dockable previous, Dockable next, boolean station) {
    try {
      access.arm();
      DockUtilities.checkLayoutLocked();
      if (previous == null) throw new NullPointerException("previous must not be null");
      if (next == null) throw new NullPointerException("next must not be null");
      if (previous != next) {
        Leaf leaf = root().getLeaf(previous);

        if (leaf == null) throw new IllegalArgumentException("Previous is not child of this station");
        ...
        if (wasFullScreen) setFullScreen(next);
  ...
  protected void discard(DockableDisplayer displayer) {
    ...
    if (index < 0) throw new IllegalArgumentException("displayer unknown to this station: " + displayer);
  ...
  public int indexOfDockable(Dockable dockable) {
    for (int i = 0, n = dockables.size(); i < n; i++) {
      if (dockables.get(i).getDockable() == dockable) return i;
    }
  ...
  private void removeDisplayer(int index, DockHierarchyLock.Token token) {
    ...
    if (handle == fullScreenDockable) {
      ...
      if (handle == fullScreenDockable) setFullScreen(null);
    }
    ...
    try {
      if (fire) dockStationListeners.fireDockableRemoving(dockable);
      ...
      if (fire) dockStationListeners.fireDockableRemoved(dockable);
    }
    finally {
      if (fire) {
        token.release();
  ...
  private class Content extends ConfiguredBackgroundPanel {
    ...
    public void doLayout() {
      ...
      if (fullScreenDockable != null) {
        fullScreenDockable.getDisplayer().getComponent()
          .setBounds(insets.left, insets.top, getWidth() - insets.left - insets.right, getHeight() - insets.bottom - insets.top);
      }
    }
  ...
  private class FullScreenListener implements DoubleClickListener {
    ...
    public boolean process(Dockable dockable, MouseEvent event) {
      if (event.isConsumed() || !isExpandOnDoubleclick()) {
        ...
        if (dockable == SplitDockStation.this) return false;
