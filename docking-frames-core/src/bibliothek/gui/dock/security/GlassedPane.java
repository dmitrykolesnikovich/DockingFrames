 ...
 */
@Todo(compatibility = Compatibility.COMPATIBLE, priority = Priority.MAJOR, target = Version.VERSION_1_1_2, description = "In Java 1.7 if a mouse-dragged is followed by a mouse-exit, and the mouse is over another GlassedPane, then this GlassedPane no longer receives events that it received in Java 1.6")
public class GlassedPane extends JPanel {
  /**
   * the strategy used by a {@link GlassedPane} to manage its tooltips
   */
  public static final PropertyKey<TooltipStrategy> TOOLTIP_STRATEGY =
    new PropertyKey<TooltipStrategy>("tooltip strategy", new ConstantPropertyFactory<TooltipStrategy>(new DefaultTooltipStrategy()), true);
  ...
  public class GlassPane extends JPanel implements MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    public void mouseExited(MouseEvent e) {
      if (!e.isConsumed() && isVisible()) send(e);
    ...
    private void sendNow(MouseEvent e, int id) {
      ...
      else if (drag) component = dragged;
      ...
      if (moved || entered || exited) {
        if (over != component) {
          ...
          if (over != null) {
            ...
            over.dispatchEvent(
              new MouseEvent(over, MouseEvent.MOUSE_EXITED, e.getWhen(), e.getModifiers(), overMouse.x, overMouse.y, e.getClickCount(),
          ...
          if (over != null) {
            Point overMouse = SwingUtilities.convertPoint(this, mouse, over);
            over.dispatchEvent(
              new MouseEvent(over, MouseEvent.MOUSE_ENTERED, e.getWhen(), e.getModifiers(), overMouse.x, overMouse.y, e.getClickCount(),
                             e.isPopupTrigger(), e.getButton()));
          }
        }
      }
      ...
      if (component == null) {
        ...
        MouseEvent forward =
          new MouseEvent(component, id, e.getWhen(), e.getModifiers(), mouse.x, mouse.y, e.getClickCount(), e.isPopupTrigger(),
                         e.getButton());
    ...
    private void send(MouseWheelEvent e) {
      ...
      if (component != null) {
        mouse = SwingUtilities.convertPoint(this, mouse, component);
        MouseWheelEvent forward =
          new MouseWheelEvent(component, e.getID(), e.getWhen(), e.getModifiers(), mouse.x, mouse.y, e.getClickCount(), e.isPopupTrigger(),
