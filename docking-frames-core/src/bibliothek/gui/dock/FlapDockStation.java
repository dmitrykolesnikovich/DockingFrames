...
public class FlapDockStation extends AbstractDockableStation {
  ...
  public static final PropertyKey<FlapLayoutManager> LAYOUT_MANAGER =
    new PropertyKey<FlapLayoutManager>("flap dock station layout manager", new DynamicPropertyFactory<FlapLayoutManager>() {
   ...
   */
  public static final PropertyKey<ButtonContent> BUTTON_CONTENT = new PropertyKey<ButtonContent>("flap dock station button content",
                                                                                                 new ConstantPropertyFactory<ButtonContent>(
                                                                                                   ButtonContent.THEME_DEPENDENT), true);
  /**
  ...
  public static final PropertyKey<ButtonContentFilter> BUTTON_CONTENT_FILTER =
    new PropertyKey<ButtonContentFilter>("flap dock station button content connector",
                                         new ConstantPropertyFactory<ButtonContentFilter>(new DefaultButtonContentFilter()), true);
  /**
   * Key for the minimum size of all {@link FlapDockStation}s.
   */
  public static final PropertyKey<Dimension> MINIMUM_SIZE =
    new PropertyKey<Dimension>("flap dock station empty size", new ConstantPropertyFactory<Dimension>(new Dimension(0, 0)), true);
  /**
  ...
  public static final PropertyKey<FlapWindowFactory> WINDOW_FACTORY = new PropertyKey<FlapWindowFactory>("flap dock station window factory",
                                                                                                         new ConstantPropertyFactory<FlapWindowFactory>(
  ...
  private PropertyValue<PlaceholderStrategy> placeholderStrategy =
    new PropertyValue<PlaceholderStrategy>(PlaceholderStrategy.PLACEHOLDER_STRATEGY) {
      @Override
  ...
  public void setController(DockController controller) {
    if (getController() != controller) {
      ...
      if (holdAction != null) holdAction.setController(controller);
      ...
      if (controller != null) {
        ...
        if (isStationShowing()) setFrontDockable(oldFrontDockable);
      }
  ...
  protected void updateWindowBounds() {
    if (window != null) window.updateBounds();
  }
  ...
  public void setAutoDirection(boolean autoDirection) {
    ...
    if (autoDirection) selfSetDirection();
  }
  ...
  private void updateWindow(Dockable dockable, boolean forceReplace) {
    if (dockable == null) {
      ...
      if (window == null || forceReplace || !windowFactory.getValue().isValid(window, this)) {
        ...
        if (window != null) setFlapWindow(window);
      }

      if (window != null && owner != null) {
        window.setDockable(dockable);
        if (owner.isVisible()) window.setWindowVisible(true);
  ...
  public void setHold(Dockable dockable, boolean hold) {
    ...
    if (manager != null) {
      ...
      if (old != hold) updateHold(dockable);
    }
  }
  ...
  public void updateHold(Dockable dockable) {
    ...
    if (manager != null) {
      ...
      if (!hold && getController() != null && getFrontDockable() == dockable) {
        if (!getController().isFocused(dockable)) setFrontDockable(null);
      }
    }
  }
  ...
  public void setWindowBorder(int windowBorder) {
    if (windowBorder < 0) throw new IllegalArgumentException("Border must not be less than 0");
  ...
  public void setWindowMinSize(int windowMinSize) {
    if (windowMinSize < 0) throw new IllegalArgumentException("Min size must not be smaller than 0");
  ...
  public void setWindowSize(Dockable dockable, int size) {
    if (size < 0) throw new IllegalArgumentException("Size must at least be 0");
  ...
  private void setDropInfo(FlapDropInfo info) {
    this.dropInfo = info;
    if (window != null) window.setDropInfo(info);

    if (buttonPane != null) buttonPane.setDropInfo(info);
  }
  ...
  public StationDropOperation prepareDrop(StationDropItem item) {
    ...
    if (dropInfo != null && dockable == getFrontDockable()) return null;
   ...
   */
  private FlapDropInfo prepareCombine(Dockable dockable,
                                      FlapWindow window,
                                      final Point mouseOnScreen,
                                      final boolean mouseOverTitle,
                                      Enforcement force) {
  ...
  public boolean drop(Dockable dockable, DockableProperty property) {
    if (property instanceof FlapDockProperty) return drop(dockable, (FlapDockProperty)property);
  ...
  public boolean drop(final Dockable dockable, FlapDockProperty property) {
    ...
    if (placeholder != null && !result) {
      ...
      if (listIndex >= 0) {
        ...
        if (size >= getWindowMinSize()) setWindowSize(dockable, size);
        result = true;
      }
      else {
  ...
  public void move(Dockable dockable, DockableProperty property) {
    ...
    if (property instanceof FlapDockProperty) {
      int index = indexOf(dockable);
      if (index < 0) throw new IllegalArgumentException("dockable is not child of this station");
  ...
  public void remove(int index) {
    ...
    if (getFrontDockable() == dockable) setFrontDockable(null);

    if (oldFrontDockable == dockable) oldFrontDockable = null;
  ...
  public boolean combine(final Dockable child, Dockable append, DockableProperty property) {
    ...
    if (index < 0) throw new IllegalArgumentException("Child must be a child of this station");
  ...
  private boolean combine(CombinerSource source, CombinerTarget target, DockableProperty property) {
    ...
    try {
      if (controller != null) controller.freezeLayout();

      int index = indexOf(child);
      if (index < 0) throw new IllegalArgumentException("old dockable must be a child of this station");

      if (append.getDockParent() != null) append.getDockParent().drag(append);
      ...
      if (controller != null) controller.meltLayout();
    }
  }
  ...
  private void replace(Dockable child, Dockable append, boolean station) {
    ...
    try {
      ...
      if (open) setFrontDockable(append);
    }
    finally {
      if (controller != null) controller.meltLayout();
    }
  ...
  private void checkShowing() {
    ...
    if (showing != lastShowing) {
      ...
      if (showing) {
        if (oldFrontDockable != null) setFrontDockable(oldFrontDockable);
      }
      else {
        oldFrontDockable = getFrontDockable();
        setFrontDockable(null);
        if (!isHold(oldFrontDockable)) oldFrontDockable = null;
      }
  ...
  private class ControllerListener implements FocusVetoListener, DockableFocusListener {
    ...
    public void dockableFocused(DockableFocusEvent event) {
      ...
      if (isStationShowing()) {
        if (front == null || (front != null && isHold(front))) return;

        DockController controller = event.getController();
        Dockable dockable = event.getNewFocusOwner();

        if (controller.isFocused(FlapDockStation.this)) return;
