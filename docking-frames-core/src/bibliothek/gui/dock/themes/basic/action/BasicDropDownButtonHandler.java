 ...
 */
public class BasicDropDownButtonHandler extends AbstractBasicHandler<DropDownAction, BasicDropDownButtonModel>
  ...
  private PropertyValue<ButtonContentFilter> buttonContentFilter =
    new PropertyValue<ButtonContentFilter>(DockAction.BUTTON_CONTENT_FILTER) {
      @Override
  ...
  public void bind() {
    ...
    if (selection != null) selection.getView().setView(selectionView);
  ...
  public void setBackground(Color background) {
    ...
    if (item != null) item.setBackground(background);
  }

  public void setForeground(Color foreground) {
    Component item = getItem();
    if (item != null) item.setForeground(foreground);
  }
  ...
  public void updateUI() {
    if (menu != null) SwingUtilities.updateComponentTreeUI(menu);
  }
  ...
  protected void update() {
    if (filter != null) filter.update(selection == null ? null : selection.getView());
  }
  ...
  private class Listener implements StandardDockActionListener, DropDownActionListener, DockActionSourceListener {
    public void actionEnabledChanged(StandardDockAction action, Set<Dockable> dockables) {
      Dockable dockable = getDockable();
      if (dockables.contains(dockable)) getModel().setEnabled(action.isEnabled(dockable));
    }

    public void actionRepresentativeChanged(StandardDockAction action, Set<Dockable> dockables) {
      if (dockables.contains(getDockable())) update();
    }
    ...
    public void selectionChanged(DropDownAction action, Set<Dockable> dockables, DockAction newSelection) {
      if (selection != null) selection.getView().setView(null);
