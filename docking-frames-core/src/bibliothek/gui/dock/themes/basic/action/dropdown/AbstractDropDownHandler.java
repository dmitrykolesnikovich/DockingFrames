 ...
 */
public abstract class AbstractDropDownHandler<S extends StandardDropDownItemAction> extends AbstractMenuHandler<JMenuItem, S>
  ...
  private class Listener implements StandardDockActionListener {
    public void actionEnabledChanged(StandardDockAction action, Set<Dockable> dockables) {
      if (view != null && dockables.contains(dockable)) view.setEnabled(action.isEnabled(dockable));
    }

    public void actionRepresentativeChanged(StandardDockAction action, Set<Dockable> dockables) {
      if (view != null && dockables.contains(dockables)) view.setDockableRepresentation(action.getDockableRepresentation(dockable));
    }
    ...
    public void actionTextChanged(StandardDockAction action, Set<Dockable> dockables) {
      if (view != null && dockables.contains(dockable)) view.setText(action.getText(dockable));
    }

    public void actionTooltipTextChanged(StandardDockAction action, Set<Dockable> dockables) {
      if (view != null && dockables.contains(dockable)) view.setTooltip(action.getTooltipText(dockable));
    }
