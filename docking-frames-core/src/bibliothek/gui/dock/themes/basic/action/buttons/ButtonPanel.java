...
public class ButtonPanel extends JPanel {
   ...
   */
  private PropertyValue<DockActionImportanceOrder> actionOrder =
    new PropertyValue<DockActionImportanceOrder>(DockActionImportanceOrder.ORDER) {
      @Override
  ...
  public void setOrientation(Orientation orientation) {
    if (this.orientation != orientation) {
      ...
      if (menuItem != null) menuItem.setOrientation(orientation);

      for (ActionItem item : actions) {
        if (item.item != null) item.item.setOrientation(orientation);
      }
  ...
  public void set(Dockable dockable, DockActionSource source, boolean force) {
    if ((source == null) != (dockable == null)) throw new IllegalArgumentException("Either both arguments are null, or none");
  ...
  public void setForeground(Color fg) {
    ...
    if (menuItem != null) menuItem.setForeground(fg);
    if (actions != null) {
      for (ActionItem item : actions) {
        if (item.item != null) item.item.setForeground(fg);
      }
    }
  }
  ...
  public void setBackground(Color bg) {
    ...
    if (menuItem != null) menuItem.setBackground(bg);

    if (actions != null) {
      for (ActionItem item : actions) {
        if (item.item != null) item.item.setBackground(bg);
      }
    }
  }
  ...
  public Dimension getMinimumSize() {
    if (actions.isEmpty()) return new Dimension(0, 0);

    if (menuItem == null) return getPreferredSize();
  ...
  public Dimension[] getPreferredSizes() {
    if (actions.isEmpty()) return new Dimension[]{new Dimension(0, 0)};

    if (menuAction == null) return new Dimension[]{getPreferredSize()};
