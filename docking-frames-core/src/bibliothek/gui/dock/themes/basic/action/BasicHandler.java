 ...
 */
public abstract class BasicHandler<D extends StandardDockAction> extends AbstractBasicHandler<D, BasicButtonModel>
  ...
  private void updateTooltip() {
    ...
    if (tooltip == null || tooltip.length() == 0) tooltip = action.getText(dockable);
  ...
  protected class Listener implements StandardDockActionListener {
    ...
    public void actionTooltipTextChanged(StandardDockAction action, Set<Dockable> dockables) {
      Dockable dockable = getDockable();
      if (dockables.contains(dockable)) updateTooltip();
    }

    public void actionRepresentativeChanged(StandardDockAction action, Set<Dockable> dockables) {
      Dockable dockable = getDockable();
      if (dockables.contains(dockable)) getModel().setDockableRepresentative(action.getDockableRepresentation(dockable));
    }
  }
