...
public class DockSituation {
  ...
  public DockSituation(DockController controller) {
    ...
    @SuppressWarnings("rawtypes") List<DockFactory> factories =
      controller.getExtensions().load(new ExtensionName<DockFactory>(DOCK_FACTORY_EXTENSION, DockFactory.class, EXTENSION_PARAM, this));
    for (DockFactory<?, ?, ?> factory : factories) {
      add(factory);
    }

    @SuppressWarnings("rawtypes") List<AdjacentDockFactory> adjacent = controller.getExtensions()
      .load(new ExtensionName<AdjacentDockFactory>(ADJACENT_DOCK_FACTORY_EXTENSION, AdjacentDockFactory.class, EXTENSION_PARAM, this));
  ...
  public DockLayoutComposition convert(DockElement element) {
    if (ignoreElement(element)) return null;

    String id = getID(element);
    DockFactory<DockElement, ?, Object> factory = (DockFactory<DockElement, ?, Object>)getFactory(id);
    if (factory == null) throw new IllegalArgumentException("Unknown factory-id: " + element.getFactoryID());
  ...
  private void writeCompositionStream(DockLayoutComposition composition, DataOutputStream out) throws IOException {
    ...
    if (info.getKind() == DockLayoutInfo.Data.BYTE) {
      ...
      DockLayout<?> layout = info.getDataLayout();
      DockFactory<DockElement, ?, Object> factory = (DockFactory<DockElement, ?, Object>)getFactory(layout.getFactoryID());
      if (factory == null) throw new IOException("Missing factory: " + layout.getFactoryID());
    ...
    if (adjacentLayouts == null) {
      ...
      for (DockLayout<?> adjacentLayout : adjacentLayouts) {
        AdjacentDockFactory<Object> adjacentFactory = (AdjacentDockFactory<Object>)getAdjacentFactory(adjacentLayout.getFactoryID());
        if (adjacentFactory == null) throw new IOException("Missing adjacent factory: " + adjacentLayout.getFactoryID());
  ...
  private byte[] readBuffer(DataInputStream in) throws IOException {
    ...
    for (int i = 0; i < count; i++) {
      int read = in.read();
      if (read == -1) throw new EOFException("unexpectetly reached end of file");
      dout.write(read);
    }
  ...
  public void writeCompositionXML(DockLayoutComposition composition, XElement element) {
    ...
    if (info.getKind() == DockLayoutInfo.Data.XML) {
      ...
      DockFactory<DockElement, ?, Object> factory = (DockFactory<DockElement, ?, Object>)getFactory(layout.getFactoryID());
      if (factory == null) throw new IllegalArgumentException("Missing factory: " + layout.getFactoryID());
    ...
    if (adjacentLayouts != null) {
      ...
      for (DockLayout<?> adjacentLayout : adjacentLayouts) {
        AdjacentDockFactory<Object> adjacentFactory = (AdjacentDockFactory<Object>)getAdjacentFactory(adjacentLayout.getFactoryID());
        if (adjacentFactory == null) throw new IllegalArgumentException("Missing adjacent factory: " + adjacentLayout.getFactoryID());
  ...
  public Map<String, DockStation> readXML(XElement root) {
    ...
    for (XElement xelement : root.getElements("element")) {
      ...
      DockElement element = composition == null ? null : convert(composition);
      DockStation station = element == null ? null : element.asDockStation();
      if (station != null) result.put(name, station);
    }
    return result;
  }
   ...
   */
  @Todo(compatibility = Compatibility.BREAK_MINOR, priority = Priority.MAJOR, target = Todo.Version.VERSION_1_1_3, description = "remove the legacy code that filters out identifiers that look like 'secure ...'. Instead 'result' can be returned directly.")
