...
public class PredefinedDockSituation extends DockSituation {
  ...
  public void put(String key, DockElement element) {
    if (stringToElement.containsKey(key)) throw new IllegalArgumentException("Key does already exist: " + key);
  ...
  protected DockLayoutInfo fillMissing(DockLayoutInfo info) {
    ...
    if (KNOWN.equals(layout.getFactoryID())) {
      ...
      if (newDelegate != null) {
        info = new DockLayoutInfo(new DockLayout<PredefinedLayout>(KNOWN, new PredefinedLayout(preloaded.getPredefined(), newDelegate)));
      }
    }
    return info;
  }
  ...
  private String getKey(DockLayoutComposition composition, boolean missingOnly) {
    ...
    if (layout.getKind() != DockLayoutInfo.Data.DOCK_LAYOUT) return null;

    if (!KNOWN.equals(layout.getDataLayout().getFactoryID())) return null;
  ...
  private DockLayoutInfo fillMissingXML(PredefinedLayout layout) {
    ...
    if (factory == null) {
      ...
      if (backup != null) {
        ...
        if (data != null) delegate = data.getData();
      }
    }
    else {
  ...
  private class PreloadFactory implements DockFactory<DockElement, PerspectiveElement, PredefinedLayout> {
    ...
    public PredefinedLayout getLayout(DockElement element, Map<Dockable, Integer> children) {
      if (shouldLayout(element)) {
        ...
        DockFactory<DockElement, ?, Object> factory = (DockFactory<DockElement, ?, Object>)getFactory(factoryId);
        if (factory == null) throw new IllegalStateException("Missing factory: " + factoryId);
    ...
    public DockElement layout(PredefinedLayout layout, Map<Integer, Dockable> children, PlaceholderStrategy placeholders) {
      ...
      if (element == null && isLayout) {
        ...
        if (factory != null) {
          return factory
            .layout(new BackupFactoryData<Object>(layout.getPredefined(), delegate.getDataLayout().getData()), children, placeholders);
        }
        return null;
      }
    ...
    public DockElement layout(PredefinedLayout layout, PlaceholderStrategy placeholders) {
      ...
      DockElement element = stringToElement.get(layout.getPredefined());
      if (element == null && isLayout) {
        if (layout.getDelegate() == null) return null;

        String factoryId = delegate.getDataLayout().getFactoryID();
        DockFactory<?, ?, BackupFactoryData<?>> factory = getBackup(factoryId);
        if (factory != null) {
          return factory.layout(new BackupFactoryData<Object>(layout.getPredefined(), delegate.getDataLayout().getData()), placeholders);
        }
    ...
    public PerspectiveElement layoutPerspective(PredefinedLayout layout, Map<Integer, PerspectiveDockable> children) {
      if (perspective == null) {
        throw new IllegalStateException(
          "the perspective of this factory is not set, meaning this factory cannot be used handling perspective dependent tasks");
      }
      ...
      if (element == null && isLayout) {
        ...
        if (factory != null) {
          return factory
            .layoutPerspective(new BackupFactoryData<Object>(layout.getPredefined(), delegate.getDataLayout().getData()), children);
        }
        return null;
      }
    ...
    public PredefinedLayout getPerspectiveLayout(PerspectiveElement element, Map<PerspectiveDockable, Integer> children) {
      ...
      if (key == null) {
        throw new IllegalStateException(
          "Expected a key for an element, the element should be known to the perspective, otherwise this method would not have been called: '" +
          element +
          "'");
      }
    ...
    public void write(PredefinedLayout layout, DataOutputStream out) throws IOException {
      ...
      if (info.getKind() == DockLayoutInfo.Data.BYTE) {
        ...
        DockFactory<DockElement, ?, Object> factory = (DockFactory<DockElement, ?, Object>)getFactory(factoryId);
        if (factory == null) throw new IOException("Missing factory: " + factoryId);
        ...
        throw new IllegalArgumentException(
          "Cannot store information as byte[], it is not present as raw byte[] or in an understandable format");
      }
    }
    ...
    public void write(PredefinedLayout layout, XElement element) {
      ...
      if (info.getKind() == DockLayoutInfo.Data.XML) {
        ...
        String factoryId = delegate.getFactoryID();
        DockFactory<DockElement, ?, Object> factory = (DockFactory<DockElement, ?, Object>)getFactory(factoryId);
        if (factory == null) throw new XException("Missing factory: " + factoryId);
        ...
        throw new IllegalArgumentException(
          "Cannot store information as xml, it is neither present as raw xml nor in an understandable format");
      }
    }
