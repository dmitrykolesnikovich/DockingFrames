...
public class PropertyTransformer {
  ...
  public PropertyTransformer(DockController controller) {
    this(controller, SplitDockPropertyFactory.FACTORY, SplitDockPathPropertyFactory.FACTORY, SplitDockPlaceholderPropertyFactory.FACTORY,
         SplitDockFullScreenPropertyFactory.FACTORY, StackDockPropertyFactory.FACTORY, FlapDockPropertyFactory.FACTORY,
  ...
  public PropertyTransformer(DockController controller, DockablePropertyFactory... factories) {
    ...
    List<DockablePropertyFactory> extensions = controller.getExtensions().load(
      new ExtensionName<DockablePropertyFactory>(FACTORY_EXTENSION, DockablePropertyFactory.class, FACTORY_EXTENSION_PARAMETER, this));
  ...
  public DockableProperty read(DataInputStream in) throws IOException {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (factory == null) throw new IOException("Unknown factory-id: " + id);
  ...
  public DockableProperty readXML(XElement element) {
    ...
    for (XElement xnode : element.getElements("property")) {
      ...
      if (factory == null) throw new IllegalArgumentException("Missing factory: " + xnode.getString("factory"));
