...
public class DockUtilities {
  ...
  private static void visitDockable(Dockable dockable, DockVisitor visitor) {
    ...
    if (station != null) visitStation(station, visitor);
  }
  ...
  public static boolean isAncestor(DockElement ancestor, DockElement child) {
    if (ancestor == null) throw new NullPointerException("ancestor must not be null");
    ...
    while (dockable != null) {
      if (ancestor == dockable) return true;
  ...
  public static DockStation getRoot(DockElement element) {
    Dockable dockable = element.asDockable();
    if (dockable == null) return element.asDockStation();

    DockStation parent = dockable.getDockParent();
    if (parent == null) return element.asDockStation();

    while (true) {
      dockable = parent.asDockable();
      if (dockable == null || dockable.getDockParent() == null) return parent;
  ...
  public static DockableProperty getLastProperty(DockableProperty property) {
    while (property.getSuccessor() != null) property = property.getSuccessor();
  ...
  public static DockableProperty getPropertyChain(Dockable dockable) {
    ...
    if (station == null || station == dockable) return null;
  ...
  public static DockableProperty getPropertyChain(DockStation ground, Dockable dockable) {
    if (ground == dockable) throw new IllegalArgumentException("ground and dockable are the same");
    ...
    while (true) {
      if (parent == ground) return property;

      child = parent.asDockable();
      if (child == null) throw new IllegalArgumentException("The chain is not complete");

      parent = child.getDockParent();
      if (parent == null) throw new IllegalArgumentException("The chain is not complete");
  ...
  public static void ensureTreeValidity(DockStation newParent, Dockable newChild) {
    ...
    if (newChild == null) throw new NullPointerException("child must not be null");

    DockStation oldParent = newChild.getDockParent();

    // check no self reference
    if (newChild == newParent) throw new IllegalArgumentException("child and parent are the same");
  ...
  public static Map<String, Icon> loadIcons(String list, String path, Set<String> ignore, ClassLoader loader) {
    try {
      ...
      if (in == null) return new HashMap<String, Icon>();
