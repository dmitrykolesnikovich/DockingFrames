...
public class NullPriorityValue<T> {
  ...
  public boolean isSet(Priority priority) {
    switch (priority) {
      case CLIENT:
        return valueClientSet;
      case DEFAULT:
        return valueDefaultSet;
      case THEME:
        return valueThemeSet;
      default:
   ...
   */
  public T get() {
    if (valueClientSet) return valueClient;

    if (valueThemeSet) return valueTheme;

    if (valueDefaultSet) return valueDefault;
