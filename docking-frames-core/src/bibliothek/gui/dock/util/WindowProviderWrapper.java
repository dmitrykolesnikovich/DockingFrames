...
public class WindowProviderWrapper implements WindowProvider {
  ...
  public void addWindowProviderListener(WindowProviderListener listener) {
    ...
    if (previous == 0 && listeners.size() > 0 && delegate != null) delegate.addWindowProviderListener(this.listener);
  }

  public void removeWindowProviderListener(WindowProviderListener listener) {
    int previous = listeners.size();
    listeners.remove(listener);
    if (previous > 0 && listeners.size() == 0 && delegate != null) delegate.removeWindowProviderListener(this.listener);
  }
  ...
  public void setDelegate(WindowProvider delegate) {
    if (listeners.size() == 0) {
      ...
      if (this.delegate != null) this.delegate.removeWindowProviderListener(listener);

      this.delegate = delegate;
      if (this.delegate != null) this.delegate.addWindowProviderListener(listener);
