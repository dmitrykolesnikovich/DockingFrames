...
public abstract class FilteredDockActionSource extends AbstractDockActionSource {
  ...
  public DockAction getDockAction(int index) {
    if (index < 0) throw new IllegalArgumentException("Index out of bounds: " + index);
    ...
    if (actions == null) {
      ...
      for (int i = 0, n = source.getDockActionCount(); i < n; i++) {
        if (actions.get(i)) {
          count++;
          if (index == count) return source.getDockAction(i);
        }
      }
    }
  ...
  public int getDockActionCount() {
    ...
    if (actions == null) {
      for (DockAction action : source) {
        if (include(action)) count++;
      }
    }
    else {
      for (int i = 0, n = source.getDockActionCount(); i < n; i++) {
        if (actions.get(i)) count++;
      }
