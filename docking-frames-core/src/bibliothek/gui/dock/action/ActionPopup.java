...
public abstract class ActionPopup extends MouseInputAdapter {
  ...
  public boolean popup(Component owner, int x, int y) {
    ...
    if (dockable.getController() == null) return false;

    PopupController popup = dockable.getController().getPopupController();

    if (!popup.isAllowOnMove() && dockable.getController().getRelocator().isOnMove()) return false;

    DockActionSource actions = getActions();

    if (!popup.isAllowEmptyMenu() && actions.getDockActionCount() == 0) return false;

    if (isSuppressable() && dockable.getController().getPopupSuppressor().suppress(dockable, actions)) return false;
