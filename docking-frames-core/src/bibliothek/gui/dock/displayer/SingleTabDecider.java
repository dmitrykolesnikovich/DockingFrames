...
public interface SingleTabDecider {
  ...
  public static SingleTabDecider ALWAYS = new SingleTabDecider() {
    public boolean showSingleTab(DockStation station, Dockable dockable) {
      if (dockable.asDockStation() != null) return false;
      if (station instanceof StackDockStation) return false;
      return true;
    }
   ...
   */
  @Deprecated @Todo(target = Version.VERSION_1_1_3, priority = Priority.MINOR, compatibility = Compatibility.BREAK_MINOR, description = "remove this constant without replacement") public static SingleTabDecider
    ALLWAYS = ALWAYS;
  ...
  public static final PropertyKey<SingleTabDecider> SINGLE_TAB_DECIDER =
    new PropertyKey<SingleTabDecider>("dock.single_tab_decider", new ConstantPropertyFactory<SingleTabDecider>(NONE), true);
