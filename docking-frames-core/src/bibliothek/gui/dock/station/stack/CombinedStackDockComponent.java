...
public abstract class CombinedStackDockComponent<T extends CombinedTab, M extends CombinedMenu, I extends CombinedInfoComponent>
  extends AbstractTabPane<T, M, I> implements StackDockComponent {
  /**
  ...
  public Rectangle getAvailableArea() {
    ...
    return new Rectangle(insets.left, insets.top, Math.max(1, panel.getWidth() - insets.left - insets.right),
  ...
  public Dimension getMinimumSize() {
    ...
    Insets insets = panel.getInsets();
    if (insets != null) {
      result = new Dimension(result.width + insets.left + insets.right, result.height + insets.top + insets.bottom);
    }
    return result;
  }
