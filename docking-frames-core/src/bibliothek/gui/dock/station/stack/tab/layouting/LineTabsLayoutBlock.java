...
public class LineTabsLayoutBlock extends AbstractTabsLayoutBlock {
    ...
    for (int i = 0; i < tabs.length; i++) {
      ...
      if (bounds.contains(mouseLocation)) {
        ...
        if (horizontal) {
          ...
          exact = (mouseLocation.y >= bounds.y + overlapPrevious[i]) && (mouseLocation.y < bounds.y + bounds.height - overlapNext[i]);
        }
  ...
  private void doLayoutPreferred(AxisConversion conversion,
                                 int width,
                                 int height,
                                 Dimension[] preferreds,
                                 Tab[] tabs,
                                 ZOrder order,
                                 int[] overlapPrevious,
                                 int[] overlapNext) {
  ...
  private void doLayoutMinimum(AxisConversion conversion,
                               int width,
                               int height,
                               Dimension[] minimums,
                               Dimension[] preferreds,
    ...
    if (tabs.length > 0) {
      ...
      tabs[last]
        .setBounds(conversion.modelToView(new Rectangle(x, 0, tabWidth, sameSize ? height : Math.min(height, preferreds[last].height))));
    }
  }

  private void doLayoutShrinked(AxisConversion conversion,
                                int width,
                                int height,
                                Dimension[] minimums,
                                Tab[] tabs,
    ...
    if (last >= 0) {
      tabs[last]
        .setBounds(conversion.modelToView(new Rectangle(x, 0, width - x, sameSize ? height : Math.min(height, minimums[last].height))));
    }
  }
