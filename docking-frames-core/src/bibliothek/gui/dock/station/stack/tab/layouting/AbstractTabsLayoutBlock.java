...
public abstract class AbstractTabsLayoutBlock implements TabsLayoutBlock {
  ...
  public void insertTab(Tab tab) {
    ...
    if (index == -1) throw new IllegalArgumentException(tab.getDockable() + " is not a child of the TabPane");
  ...
  public Tab[] getTabsOrderedByImportance() {
    if (pane == null) throw new IllegalStateException("no TabPane available");
  ...
  public Tab[] getDockableTabMap() {
    TabPane pane = getPane();
    if (pane == null) throw new IllegalStateException("missing the TabPane");
