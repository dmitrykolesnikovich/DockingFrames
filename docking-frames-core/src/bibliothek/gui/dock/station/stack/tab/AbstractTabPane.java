...
public abstract class AbstractTabPane<T extends Tab, M extends TabMenu, I extends LonelyTabPaneComponent> implements TabPane {
  ...
  private PropertyValue<TabLayoutManager> layoutManager = new PropertyValue<TabLayoutManager>(TabPane.LAYOUT_MANAGER) {
    ...
    protected void valueChanged(TabLayoutManager oldValue, TabLayoutManager newValue) {
      if (oldValue != null) oldValue.uninstall(AbstractTabPane.this);

      if (newValue != null) newValue.install(AbstractTabPane.this);
    }
  };
  ...
  public void setDockTabPlacement(TabPlacement tabPlacement) {
    if (tabPlacement == null) throw new IllegalArgumentException("tab placement must not be null");
  ...
  public void remove(int index) {
    ...
    // select other tab
    if (selected) {
      if (index >= getDockableCount()) index = getDockableCount() - 1;
      if (index >= 0) setSelectedDockable(getDockable(index));
    }
  ...
  public T getVisibleTab(int index) {
    if (index < 0) throw new IllegalArgumentException("index to small");
  ...
  public T putOnTab(Dockable dockable) {
    ...
    if (!dockables.contains(dockable)) throw new IllegalArgumentException("dockable not child of this pane");
  ...
  public void setInfoComponent(I info) {
    if (this.info != info) {
      if (this.info != null) this.info.setPaneVisible(false);

      I oldInfo = this.info;
      this.info = info;

      if (this.info != null) this.info.setPaneVisible(true);
  ...
  public void putInMenu(TabMenu menu, Dockable dockable) {
    if (dockable == null) throw new IllegalArgumentException("dockables must not be null");

    if (!this.dockables.contains(dockable)) throw new IllegalArgumentException("not child of this pane: " + dockable);

    if (menu == null) throw new IllegalArgumentException("menu is null");

    if (!menus.contains(menu)) throw new IllegalArgumentException("menu not created by this pane");
  ...
  public void destroyMenu(TabMenu menu) {
    ...
    if (!menus.remove(menu)) throw new IllegalArgumentException("menu not created by this pane");
