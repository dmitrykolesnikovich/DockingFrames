...
public class MenuLineLayoutPossibility {
  ...
  public boolean isPreferred() {
    if (tabSize == null || !tabSize.isPreferred() || !getPane().getTabs().isAllTabs(tabSize)) return false;

    if (menuSize != null) return false;
  ...
  public void apply() {
    ...
    if (infoSize != null && info != null) info.setLayout(infoSize);
    if (menuSize == null) {
    ...
    if (orientation.isHorizontal()) {
      ...
      if (infoSize != null) required = Math.max(required, infoSize.getHeight());
      if (menuSize != null) required = Math.max(required, menuSize.getHeight());
    }
    else {
      required = tabSize.getWidth();
      if (infoSize != null) required = Math.max(required, infoSize.getWidth());
      if (menuSize != null) required = Math.max(required, menuSize.getWidth());
    }

    required = Math.max(0, Math.min(required, available.height / 2));

    pane.setSelectedBounds(
      conversion.modelToView(new Rectangle(available.x, available.y + required, available.width, available.height - required)));
   ...
   */
  private int[] calculateWidths(TabPlacement orientation,
                                MenuLineLayoutOrder order,
                                int available,
                                Size tabSize,
                                Size menuSize,
                                Size infoSize) {
