...
public class DefaultSplitLayoutManager implements SplitLayoutManager {
  ...
  public PutInfo prepareDrop(SplitDockStation station, StationDropItem item) {
    if (station.isFullScreen()) return null;
  ...
  protected PutInfo calculateSideSnap(SplitDockStation station, int x, int y, Leaf leaf, Dockable drop) {
    if (station.getDockableCount() == 0) return null;

    if (station.getDockableCount() == 1 && station.getDockable(0) == drop) return null;
    ...
    if (leaf != null && station.getRoot().getChild() instanceof Node) {
      ...
      if (node.getLeft().isVisible() && node.getRight().isVisible()) {
        if (info.getPut() == PutInfo.Put.TOP && node.getOrientation() == Orientation.VERTICAL && node.getLeft() == leaf) return null;

        if (info.getPut() == PutInfo.Put.BOTTOM && node.getOrientation() == Orientation.VERTICAL && node.getRight() == leaf) return null;

        if (info.getPut() == PutInfo.Put.LEFT && node.getOrientation() == Orientation.HORIZONTAL && node.getLeft() == leaf) return null;

        if (info.getPut() == PutInfo.Put.RIGHT && node.getOrientation() == Orientation.HORIZONTAL && node.getRight() == leaf) return null;
      }
    }
  ...
  public void calculateDivider(SplitDockStation station, PutInfo putInfo, Leaf origin, StationDropItem item) {
    ...
    Dimension oldSize = origin == null ? item.getOriginalSize() : origin.getSize();
    ...
    if (putInfo.getPut() == PutInfo.Put.TOP) {
      if (size != 0) divider = (size + dividerSize / 2.0) / nodeSize.height;

      divider = validateDivider(station, divider, item.getMinimumSize(), other.getMinimumSize(), Orientation.VERTICAL, other.getWidth(),
                                other.getHeight());

      if (divider > 1 - MINIMUM_ORIGINAL_SIZE) divider = 1 - MINIMUM_ORIGINAL_SIZE;
    }
    else if (putInfo.getPut() == PutInfo.Put.BOTTOM) {
      if (size != 0) divider = 1.0 - (size + dividerSize / 2.0) / nodeSize.height;

      divider = validateDivider(station, divider, other.getMinimumSize(), item.getMinimumSize(), Orientation.VERTICAL, other.getWidth(),
                                other.getHeight());

      if (divider < MINIMUM_ORIGINAL_SIZE) divider = MINIMUM_ORIGINAL_SIZE;
    }
    else if (putInfo.getPut() == PutInfo.Put.LEFT) {
      if (size != 0) divider = (size + dividerSize / 2.0) / nodeSize.width;

      divider = validateDivider(station, divider, item.getMinimumSize(), other.getMinimumSize(), Orientation.HORIZONTAL, other.getWidth(),
                                other.getHeight());

      if (divider > 1 - MINIMUM_ORIGINAL_SIZE) divider = 1 - MINIMUM_ORIGINAL_SIZE;
    }
    else if (putInfo.getPut() == PutInfo.Put.RIGHT) {
      if (size != 0) divider = 1.0 - (size + dividerSize / 2.0) / nodeSize.width;

      divider = validateDivider(station, divider, other.getMinimumSize(), item.getMinimumSize(), Orientation.HORIZONTAL, other.getWidth(),
                                other.getHeight());
  ...
  protected double validateDivider(SplitDockStation station,
    ...
    if (factor <= 0 || Double.isNaN(factor)) return divider;
    ...
    else if (divider * size > size - rightNeed - dividerNeed / 2) divider = (size - rightNeed - dividerNeed / 2) / size;
  ...
  public PutInfo validatePutInfo(SplitDockStation station, PutInfo putInfo) {
    if (putInfo != null) {
      if (!station.accept(putInfo.getDockable())) return null;
      ...
      if (putInfo.getNode() != null && (putInfo.getPut() == PutInfo.Put.CENTER || putInfo.getPut() == PutInfo.Put.TITLE)) {
        ...
        if (!putInfo.getDockable().accept(station) || !station.getController().getAcceptance().accept(station, putInfo.getDockable())) {
          return null;
        }
      }
    }
