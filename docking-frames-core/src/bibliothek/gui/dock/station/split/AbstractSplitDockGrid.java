...
public abstract class AbstractSplitDockGrid<D> {
  ...
  public void addDockable(double x, double y, double width, double height, D... dockables) {
    if (dockables == null) throw new IllegalArgumentException("Dockable must not be null");

    if (dockables.length == 0) throw new IllegalArgumentException("Dockables must at least have one element");

    for (D dockable : dockables) {
      if (dockable == null) throw new IllegalArgumentException("Entry of dockables-array is null");
    }
  ...
  public void addPlaceholders(double x, double y, double width, double height, Path... placeholders) {
    if (placeholders == null) throw new IllegalArgumentException("Placeholders must not be null");

    if (placeholders.length == 0) throw new IllegalArgumentException("Placeholders must at least have one element");

    for (Path placeholder : placeholders) {
      if (placeholder == null) throw new IllegalArgumentException("Entry of placeholders-array is null");
    }
  ...
  public void setSelected(double x, double y, double width, double height, D dockable) {
    if (dockable == null) throw new IllegalArgumentException("dockable is null");
  ...
  protected Line bestFittingLine(double x, double y, double w, double h, boolean horizontal, double split) {
    ...
    for (Line line : lines) {
      ...
      if (line.horizontal) {
        if (y > line.alpha || y + h < line.alpha) continue;

        if (x + w < line.betaMin || x > line.betaMax) continue;
        ...
        if (x > line.alpha || x + w < line.alpha) continue;

        if (y + h < line.betaMin || y > line.betaMax) continue;
  ...
  protected double penalty(double x, double y, double w, double h, Line line) {
    ...
    if (line.horizontal) {
      if (y >= line.alpha || y + h <= line.alpha) return 0;

      if (x + w <= line.betaMin || x >= line.betaMax) return 0;
      ...
      if (x >= line.alpha || x + w <= line.alpha) return 0;

      if (y + h <= line.betaMin || y >= line.betaMax) return 0;
