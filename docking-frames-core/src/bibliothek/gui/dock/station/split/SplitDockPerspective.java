...
public class SplitDockPerspective implements PerspectiveDockable, PerspectiveStation {
  ...
  private Entry convert(PerspectiveSplitDockTree.Key key) {
    ...
    if (tree.isNode(key)) {
      ...
      return new Node(orientation, tree.getDivider(key), childA, childB, toSet(tree.getPlaceholders(key)), tree.getPlaceholderMap(key),
                      key.getNodeId());
    }
  ...
  public static class Node extends Entry {
     ...
     */
    public Node(Orientation orientation,
                double divider,
                Entry childA,
                Entry childB,
