...
public abstract class SplitNode {
  ...
  protected SplitNode(SplitDockAccess access, long id) {
    if (access == null) throw new IllegalArgumentException("Access must not be null");
    this.access = access;
    if (id < 0) {
  ...
  public Path[] getPlaceholders() {
    if (placeholders == null) return new Path[]{};
  ...
  protected void split(SplitDockPathProperty property, int depth, SplitNode newChild, long newNodeId) {
    ...
    boolean reverse =
      node.getLocation() == SplitDockPathProperty.Location.RIGHT || node.getLocation() == SplitDockPathProperty.Location.BOTTOM;
  ...
  public Dimension getSize() {
    ...
    return new Dimension((int)(width * fw + 0.5), (int)(height * fh + 0.5));
  }
  ...
  public Rectangle getBounds() {
    ...
    double fw = root.getWidthFactor();
    double fh = root.getHeightFactor();
    Rectangle rec = new Rectangle((int)(x * fw + 0.5), (int)(y * fh + 0.5), (int)(width * fw + 0.5), (int)(height * fh + 0.5));
  ...
  protected Leaf create(Dockable dockable, long id) {
    ...
    if (!dockable.accept(split) || !split.accept(dockable)) return null;

    if (acceptance != null) {
      if (!acceptance.accept(split, dockable)) return null;
    }
        ...
        if (dockables.length == 1) {
          ...
          if (dockables.length == 2) {
            ...
            if (station != null) {
              ...
              if (selected != null) station.setFrontDockable(selected);
            }
          }
          else {
            DockStation station = combination.asDockStation();
