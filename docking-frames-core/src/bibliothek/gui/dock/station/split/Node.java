...
public class Node extends VisibleSplitNode implements Divideable {
  ...
  public int getChildLocation(SplitNode child) {
    if (left == child) return 0;
    if (right == child) return 1;
  ...
  public void setOrientation(Orientation orientation) {
    if (orientation == null) throw new NullPointerException("orientation must not be null");
    this.orientation = orientation;
    getAccess().getOwner().revalidate();
  }
  ...
  private Dimension getSize(Dimension left, Dimension right) {
    if (left != null && right != null) {
      ...
      if (orientation == Orientation.HORIZONTAL) {
        return new Dimension(left.width + divider + right.width, Math.max(left.height, right.height));
      }
      else {
        return new Dimension(Math.max(left.width, right.width), left.height + divider + right.height);
      }
    }
    else if (left != null) {
      return left;
  ...
  public void updateBounds(double x, double y, double width, double height, double factorW, double factorH, boolean components) {
    ...
    if (leftVisible && !rightVisible) {
      ...
      if (orientation == Orientation.HORIZONTAL) {
        ...
        if (left != null) left.updateBounds(x, y, dividerLocation - dividerWidth / 2, height, factorW, factorH, components);

        if (right != null) {
          right
            .updateBounds(x + dividerLocation + dividerWidth / 2, y, width - dividerLocation - dividerWidth / 2, height, factorW, factorH,
                          components);
        }
      }
      else {
        ...
        if (left != null) left.updateBounds(x, y, width, dividerLocation - dividerHeight / 2, factorW, factorH, components);

        if (right != null) {
          right
            .updateBounds(x, y + dividerLocation + dividerHeight / 2, width, height - dividerLocation - dividerHeight / 2, factorW, factorH,
                          components);
        }
      }
    }
  ...
  public void setBounds(double x, double y, double width, double height, double factorW, double factorH, boolean updateComponentBounds) {
    ...
    if (leftVisible && rightVisible) {
      ...
      if (orientation == Orientation.HORIZONTAL) {
        ...
        dividerBounds.setBounds((int)((x + dividerLocation - dividerWidth / 2) * factorW), (int)(y * factorH), dividerSize,
        ...
        dividerBounds
          .setBounds((int)(x * factorW), (int)((y + dividerLocation - dividerHeight / 2) * factorH), (int)(width * factorW + 0.5),
                     dividerSize);
      }
    }
  }
  ...
  public Rectangle getDividerBounds(double divider, Rectangle bounds) {
    ...
    if (orientation == Orientation.HORIZONTAL) {
      ...
      bounds.setBounds((int)((x + dividerLocation - dividerWidth / 2) * factorW + 0.5), (int)(y * factorH + 0.5), dividerSize,
                       (int)(height * factorH + 0.5));
    }
    else {
      ...
      bounds
        .setBounds((int)(x * factorW + 0.5), (int)((y + dividerLocation - dividerHeight / 2) * factorH + 0.5), (int)(width * factorW + 0.5),
                   dividerSize);
    }
  ...
  private boolean needToExpand(SplitDockPathProperty.Node node) {
    boolean expand =
      ...
      (orientation == SplitDockStation.Orientation.HORIZONTAL &&
       (node.getLocation() == SplitDockPathProperty.Location.TOP || node.getLocation() == SplitDockPathProperty.Location.BOTTOM)) ||
      // ... or if this node is vertical, but the path is horizontal
      (orientation == SplitDockStation.Orientation.VERTICAL &&
       (node.getLocation() == SplitDockPathProperty.Location.LEFT || node.getLocation() == SplitDockPathProperty.Location.RIGHT));
  ...
  public <N> N submit(SplitTreeFactory<N> factory) {
    if (orientation == SplitDockStation.Orientation.HORIZONTAL) {
      return factory
        .horizontal(left.submit(factory), right.submit(factory), divider, getId(), getPlaceholders(), getPlaceholderMap(), isVisible());
    }
    else {
      return factory
  ...
  public Node getDividerNode(int x, int y) {
    ...
    if (leftVisible && rightVisible && dividerBounds.contains(x, y)) return this;
