...
public abstract class SplitDockTree<D> {
  ...
  public SplitDockTree<D> root(Key key) {
    ...
    if (key.getTree() != this) throw new IllegalArgumentException("Key is not created by this tree");

    if (key.parent != null) throw new IllegalArgumentException("Key has a parent, and can't be the root");
  ...
  public Key put(D[] dockables, D selected, Path[] placeholders, PlaceholderMap placeholderMap, long nodeId) {
    if (placeholders == null || placeholders.length == 0) {
      if (dockables == null) throw new IllegalArgumentException("Dockables must not be null");

      if (dockables.length == 0) throw new IllegalArgumentException("At least one Dockable is required");
    }

    if (dockables != null) {
      for (D dockable : dockables) {
        if (dockable == null) throw new IllegalArgumentException("Entries of array must not be null");

        if (!this.dockables.add(dockable)) throw new IllegalArgumentException("Dockable already known");
      }
    }
  ...
  public D[] getDockables(Key key) {
    if (!isDockable(key)) throw new IllegalArgumentException("Not a Dockable");
  ...
  public boolean isHorizontal(Key key) {
    if (!isNode(key)) throw new IllegalArgumentException("Not a node");
  ...
  private class Node extends Key {
    ...
    public Node(Key keyA, Key keyB, double divider, boolean horizontal, Path[] placeholders, PlaceholderMap placeholderMap, long id) {
      super(placeholders, placeholderMap, id);
      if (keyA.getTree() != getTree()) throw new IllegalArgumentException("Key of first argument belongs not to this tree");

      if (keyB.getTree() != getTree()) throw new IllegalArgumentException("Key of second argument belongs not to this tree");

      if (divider < 0 || divider > 1.0) throw new IllegalArgumentException("Divider out of bounds, must be between 0 and 1");

      if (keyA.getParent() != null) throw new IllegalArgumentException("First key already has a parent");

      if (keyB.getParent() != null) throw new IllegalArgumentException("Second key already has a parent");

      if (keyA == keyB) throw new IllegalArgumentException("The arguments must not be the same object");

      if (keyA == root) throw new IllegalArgumentException("First argument is the root, can't be a child of any other node");

      if (keyB == root) throw new IllegalArgumentException("Second argument is the root, can't be a child of any other node");
