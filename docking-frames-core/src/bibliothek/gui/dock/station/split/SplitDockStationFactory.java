...
public class SplitDockStationFactory implements DockFactory<SplitDockStation, SplitDockPerspective, SplitDockStationLayout> {
  ...
  public SplitDockStationLayout getLayout(final SplitDockStation station, final Map<Dockable, Integer> children) {
    Entry root = station.visit(new SplitTreeFactory<Entry>() {
      ...
      public Entry horizontal(Entry left,
                              Entry right,
                              double divider,
                              long id,
                              Path[] placeholders,
      ...
      public Entry vertical(Entry top,
                            Entry bottom,
                            double divider,
                            long id,
                            Path[] placeholders,
    ...
    if (fullscreenDockable != null) fullscreen = children.get(fullscreenDockable);
  ...
  private Entry convert(SplitDockPerspective.Entry entry, Map<PerspectiveDockable, Integer> children) {
    ...
    if (entry.asNode() != null) {
      ...
      return new SplitDockStationLayout.Node(node.getOrientation(), node.getDivider(), childA, childB, toArray(node.getPlaceholders()),
                                             node.getPlaceholderMap(), node.getNodeId());
    }
    else if (entry.asLeaf() != null) {
      SplitDockPerspective.Leaf leaf = entry.asLeaf();
      Integer id = children.get(leaf.getDockable());
      return new SplitDockStationLayout.Leaf(id == null ? -1 : id.intValue(), toArray(leaf.getPlaceholders()), leaf.getPlaceholderMap(),
                                             leaf.getNodeId());
    }
    else {
   ...
   * @throws IOException if an I/O-error occurs
   */
  private SplitDockStationLayout.Entry readEntry(DataInputStream in, boolean version8, boolean version8a, PlaceholderStrategy strategy)
    throws IOException {
    long id = -1;
    if (version8) {
      id = in.readLong();
  ...
  public SplitDockStationLayout read(XElement element, PlaceholderStrategy placeholders) {
    ...
    if (xroot == null) xroot = element.getElement("leaf");
    if (xroot != null) root = readEntry(xroot, placeholders);

    int fullscreen = -1;
    XElement xfullscreen = element.getElement("fullscreen");
    if (xfullscreen != null) fullscreen = xfullscreen.getInt("id");
  ...
  private SplitDockStationLayout.Entry readEntry(XElement element, PlaceholderStrategy strategy) {
    ...
    if ("node".equals(element.getName())) {
      ...
      if (xchildren.length != 2) throw new XException("node element must have exactly least two children: " + element);


      return new SplitDockStationLayout.Node(Orientation.valueOf(element.getString("orientation")), element.getDouble("divider"),
                                             readEntry(xchildren[0], strategy), readEntry(xchildren[1], strategy), placeholders, map,
                                             nodeId);
    }
