...
public class SplitDockPlaceholderProperty extends AbstractDockableProperty {
  ...
  private SplitDockPlaceholderProperty(Path placeholder, DockableProperty backup) {
    if (placeholder == null) throw new IllegalArgumentException("placeholder must not be null");
  ...
  public SplitDockProperty toSplitLocation(SplitNode target) {
    if (backup instanceof SplitDockProperty) return (SplitDockProperty)backup;

    if (backup instanceof SplitDockPathProperty) return ((SplitDockPathProperty)backup).toLocation(target);
