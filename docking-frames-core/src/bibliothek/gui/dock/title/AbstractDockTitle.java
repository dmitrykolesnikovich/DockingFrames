...
public class AbstractDockTitle extends AbstractMultiDockTitle {
  ...
  protected void setTooltip(String text) {
    ...
    if (itemPanel != null) itemPanel.setToolTipText(text);
  }
  ...
  protected void doTitleLayout() {
    ...
    if (orientation.isHorizontal()) {
      ...
      if (itemPanel != null && itemPanel.getItemCount() > 0) {
        ...
        while (count > 0 && buttonPreferred[count].width > remaining) count--;
      ...
      if (itemPanel != null && itemPanel.getItemCount() > 0) {
        ...
        while (count > 0 && buttonPreferred[count].height > remaining) count--;
    ...
    if (icon != null) {
      if (icon.contains(click)) {
        if (restrained) {
          ...
          if (itemPanel != null) size -= itemPanel.getWidth() * itemPanel.getHeight();

          if (size <= 2 * icon.width * icon.height) return null;
        }
