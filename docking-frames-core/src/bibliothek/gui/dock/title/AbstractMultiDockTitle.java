...
public abstract class AbstractMultiDockTitle extends ConfiguredBackgroundPanel implements DockTitle {
  ...
  private PropertyValue<OrientationToRotationStrategy> orientationConverter =
    new PropertyValue<OrientationToRotationStrategy>(DockTitle.ORIENTATION_STRATEGY) {
  ...
  protected void addConditionalFont(String id, Path kind, Condition condition, FontModifier backup) {
    ...
    addFont(font);
    if (conditionalFonts == null) conditionalFonts = new ArrayList<ConditionalFont>();
  ...
  public void setBackground(Color fg) {
    ...
    if (label != null) label.setBackground(fg);
  }
  ...
  public Dimension getPreferredSize() {
    ...
    if (orientation.isHorizontal()) {
      ...
      if (icon == null) width = Math.max(width, 2 * height);
      ...
      if (icon == null) height = Math.max(height, 2 * width);
  ...
  public void bind() {
    if (bound) throw new IllegalArgumentException("Do not call bound twice!");
    bound = true;
  ...
  public void unbind() {
    if (!bound) throw new IllegalArgumentException("Do not call unbind twice");
    bound = false;
