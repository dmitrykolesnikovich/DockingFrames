...
public class DockTitleVersion implements DockTitleFactory {
  ...
  public DockTitleVersion(DockController controller, String id) {
    ...
    List<DockTitleFactory> list = controller.getExtensions().load(
      new ExtensionName<DockTitleFactory>(DOCK_TITLE_VERSION_EXTENSION, DockTitleFactory.class, DOCK_TITLE_VERSION_EXTENSION_PARAMETER,
                                          this));
    if (!list.isEmpty()) {
  ...
  public DockTitleFactory getFactory() {
    for (int i = 0; i < factories.length; i++) {
      if (factories[i] != null) return factories[i];
    }
