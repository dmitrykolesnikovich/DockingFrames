...
public class SingleParentRemover {
  ...
  protected boolean test(DockStation station) {
    ...
    try {
      ...
      if (!shouldTest(station)) return false;

      if (station.getDockableCount() > 1) return false;
      ...
      if (parent.getController() == null) return false;

      if (station.getDockableCount() == 0) {
        if (!parent.canDrag(transform)) return false;
        ...
        if (!station.canDrag(dockable)) return false;

        if (!parent.accept(dockable) || !dockable.accept(parent)) return false;

        if (!parent.canReplace(transform, dockable)) return false;

        DockAcceptance acceptance = station.getController().getAcceptance();

        if (acceptance != null) {
          if (!acceptance.accept(parent, dockable)) return false;
        }
