...
public class DefaultDockRelocator extends AbstractDockRelocator {
  ...
  public static final PropertyKey<Boolean> AUTO_DROP_ON_ANY_MOUSE_RELEASED_EVENT =
    new PropertyKey<Boolean>("dock.default.relocator.autodrop", new ConstantPropertyFactory<Boolean>(true), true);
  ...
  public DefaultDockRelocator(DockController controller, ControllerSetupCollection setup) {
    ...
    setup.add(new ControllerSetupListener() {
      public void done(DockController controller) {
        ...
        List<Merger> mergers = controller.getExtensions()
          .load(new ExtensionName<Merger>(MERGE_EXTENSION, Merger.class, EXTENSION_PARAM, DefaultDockRelocator.this));
        for (Merger next : mergers) {
          merger.add(next);
        }

        List<Inserter> inserters = controller.getExtensions()
          .load(new ExtensionName<Inserter>(INSERTER_EXTENSION, Inserter.class, EXTENSION_PARAM, DefaultDockRelocator.this));
    ...
    for (DockStation station : list) {
      ...
      if (result != null) {
        ...
        DefaultDockRelocatorEvent event =
          new DefaultDockRelocatorEvent(getController(), dockable, result.getImplicit(dockable), station, new Point(mouseX, mouseY), move);
        fireSearched(event);
  ...
  protected void dragMousePressed(MouseEvent e, DockTitle title, Dockable dockable) {
    if (dockable == null) dockable = title.getDockable();

    Point point = e.getPoint();
    SwingUtilities.convertPointToScreen(point, e.getComponent());
    Reaction reaction = dragMousePressed(point.x, point.y, e.getX(), e.getY(), e.getModifiersEx(), dockable, false);
    if (reaction == Reaction.BREAK_CONSUMED || reaction == Reaction.CONTINUE_CONSUMED) e.consume();
  }
  ...
  protected Reaction dragMousePressed(int x, int y, int dx, int dy, int modifiers, Dockable dockable, boolean forceDrag) {
    ...
    if (!forceDrag && dockable.getDockParent() == null) return Reaction.BREAK;
    ...
    if (((modifiers & (onmask | offmask)) == onmask)) {
      ...
      DefaultDockRelocatorEvent event =
        new DefaultDockRelocatorEvent(getController(), dockable, implicit, operation == null ? null : operation.getStation(),
                                      new Point(x, y), move);
      event.cancel();
      fireCanceled(event);
   ...
   * @return how this relocator reacts on the event
   */
  protected Reaction dragMouseDragged(int x,
                                      int y,
                                      int modifiers,
                                      DockTitle title,
                                      Dockable dockable,
    ...
    if (pressPointScreen == null) return Reaction.BREAK;
  ...
  private Reaction selectNextTarget(Dockable dockable, DockTitle title, Point mouse) {
    if (movingImageWindow != null) updateTitleWindowPosition(mouse);
    ...
    DefaultDockRelocatorEvent event =
      new DefaultDockRelocatorEvent(getController(), dockable, implicit, next == null ? null : next.getStation(), mouse, move);
    fireDragged(event);
    if (event.isCanceled()) {
      cancel(dockable);
      return onBreak(event);
  ...
  protected Reaction dragMouseReleased(int x, int y, int modifiers, Dockable dockable) {
    ...
    if (stop) {
      if (pressPointScreen != null) {
        ...
        if (x != lastPoint.x || y != lastPoint.y) {
          ...
          if (next != null) {
            ...
            DefaultDockRelocatorEvent event =
              new DefaultDockRelocatorEvent(getController(), dockable, implicit, next.getStation(), mouse, move);
            fireCanceled(event);
            if (event.isCanceled() || event.isForbidden()) {
              next = null;
            }
        ...
        if (operation != null) {
          ...
          DefaultDockRelocatorEvent event =
            new DefaultDockRelocatorEvent(getController(), dockable, implicit, operation.getStation(), mouse, move);
          event.drop();
          fireDropping(event);
          if (event.isCanceled() || event.isForbidden()) {
            operation.destroy(null);
        ...
        if (operation != null) {
          ...
          dropped = new DefaultDockRelocatorEvent(getController(), dockable, implicit, operation.getStation(), mouse,
                                                  operation.getOperation().isMove());
          if (canceled) {
            dropped.cancel();
      ...
      if (movingImageWindow != null) movingImageWindow.close();
  ...
  private class MouseRepresentativeListener extends MouseInputAdapter {
    ...
    public MouseRepresentativeListener(Listener parent, DockElementRepresentative representative) {
      ...
      if (representative instanceof DockTitle) title = (DockTitle)representative;
    ...
    public void mousePressed(MouseEvent e) {
      ...
      if (!representative.isUsedAsTitle() && isDragOnlyTitle()) return;
