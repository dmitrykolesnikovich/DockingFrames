...
public class StackDockStation extends AbstractDockableStation implements StackDockComponentParent {
  ...
  public static final PropertyKey<TabPlacement> TAB_PLACEMENT =
    new PropertyKey<TabPlacement>("stack dock station tab side", new ConstantPropertyFactory<TabPlacement>(TabPlacement.TOP_OF_DOCKABLE),
                                  true);
  ...
  public static final PropertyKey<Boolean> IMMUTABLE_SELECTION_INDEX =
    new PropertyKey<Boolean>("stack dock immutable selection index", new ConstantPropertyFactory<Boolean>(false), true);
   ...
   */
  @FrameworkOnly public static final PropertyKey<StackDnDAutoSelectSupport> DND_AUTO_SELECT_SUPPORT =
  ...
  private PropertyValue<StackDnDAutoSelectSupport> autoSelectSupport =
    new PropertyValue<StackDnDAutoSelectSupport>(DND_AUTO_SELECT_SUPPORT) {
      @Override
  ...
  public void setStackComponent(StackDockComponent stackComponent) {
    if (stackComponent == null) throw new IllegalArgumentException("Component must not be null");
    ...
    if (stackComponent != this.stackComponent) {
      ...
      if (getDockableCount() < 2 && !singleTabStackDockComponent()) {
        ...
        if (selected >= 0 && selected < stackComponent.getTabCount()) stackComponent.setSelectedIndex(selected);
  ...
  public void setDockParent(DockStation station) {
    ...
    if (old != null) old.removeDockStationListener(visibleListener);
  ...
  public Dockable getFrontDockable() {
    if (dockables.dockables().size() == 0) return null;
    if (dockables.dockables().size() == 1) return dockables.dockables().get(0).getDockable();

    int index = stackComponent.getSelectedIndex();
    if (index >= 0) return dockables.dockables().get(index).getDockable();

    return null;
  }

  public void setFrontDockable(Dockable dockable) {
    if (dockables.dockables().size() > 1 && dockable != null) stackComponent.setSelectedIndex(indexOf(dockable));
  ...
  public void move(Dockable dockable, DockableProperty property) {
    if (property instanceof StackDockProperty) {
      ...
      int index = indexOf(dockable);
      if (index < 0) throw new IllegalArgumentException("dockable not child of this station");
  ...
  protected Insert tabIndexAt(int x, int y) {
    if (dockables.dockables().size() == 0) return new Insert(0, false);
    if (dockables.dockables().size() == 1) return new Insert(1, false);

    Insert insert = exactTabIndexAt(x, y);
    if (insert == null) insert = new Insert(dockables.dockables().size() - 1, true);
  ...
  public void drag(Dockable dockable) {
    ...
    int index = indexOf(dockable);
    if (index < 0) throw new IllegalArgumentException("The dockable is not part of this station.");
  ...
  protected void discard(DockableDisplayer displayer) {
    ...
    int index = indexOf(dockable);
    if (index < 0) throw new IllegalArgumentException("displayer is not a child of this station: " + displayer);
  ...
  public void remove(int index) {
    if (index < 0 || index >= dockables.dockables().size()) throw new IllegalArgumentException("Index out of bounds");
  ...
  protected class Background extends SecureContainer {
    ...
    protected void paintOverlay(Graphics g) {
      ...
      if (insert != null && dockables.dockables().size() > 1) {
        ...
        if (insert.tab >= 0 && insert.tab < stackComponent.getTabCount()) bounds = stackComponent.getBoundsAt(insert.tab);
        ...
        if (bounds != null) {
          ...
          if (insert.right) {
            insertionLine(bounds, insert.tab + 1 < stackComponent.getTabCount() ? stackComponent.getBoundsAt(insert.tab + 1) : null, a, b,
                          true);
          }
          else {
