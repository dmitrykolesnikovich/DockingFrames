...
public class DockFrontend {
   ...
   */
  public static final PropertyKey<KeyStroke> HIDE_ACCELERATOR = new PropertyKey<KeyStroke>("frontend hide accelerator");
   ...
   */
  private Set<DockFactory<?, ?, ?>> dockFactories = new HashSet<DockFactory<?, ?, ?>>();

  /**
   * A set of factories needed to read Dockables that are missing in the cache
   */
  private Set<DockFactory<? extends Dockable, ?, ?>> backupDockFactories = new HashSet<DockFactory<? extends Dockable, ?, ?>>();

  /**
   * A set of factories needed to store additional information about Dockables
   */
  private Set<AdjacentDockFactory<?>> adjacentDockFactories = new HashSet<AdjacentDockFactory<?>>();
   ...
   */
  @Todo(compatibility = Compatibility.COMPATIBLE, priority = Priority.MINOR, target = Todo.Version.VERSION_1_1_3, description =
    "This really is a workaround preventing loading a layout while loading another layout. The " +
    "better solution would be to not apply layouts to children that already have a layout stored " +
    "in the DockInfo-object") private boolean readingOldLayoutInformation = false;
  ...
  public void registerFactory(DockFactory<? extends Dockable, ?, ?> factory, boolean backup) {
    ...
    dockFactories.add(factory);
    if (backup) backupDockFactories.add(factory);
    fillMissing(factory);
  }
  ...
  public void addDockable(String id, Dockable dockable) {
    ...
    if (id == null) throw new IllegalArgumentException("name must not be null");
  ...
  public Map<String, Dockable> getNamedDockables() {
    ...
    for (Map.Entry<String, DockInfo> entry : dockables.entrySet()) {
      if (entry.getValue().getDockable() != null) result.put(entry.getKey(), entry.getValue().getDockable());
    }
    return result;
  }
  ...
  public String getNameOf(Dockable dockable) {
    if (dockable == null) throw new NullPointerException("dockable is null");

    for (Map.Entry<String, DockInfo> entry : dockables.entrySet()) {
      if (entry.getValue().dockable == dockable) return entry.getKey();
    }
    return null;
  }
  ...
  public void addRoot(DockStation station, String name) {
    if (station == null) throw new IllegalArgumentException("Stations must not be null");

    if (name == null) throw new IllegalArgumentException("name must not be null");

    if (roots.containsKey(name)) throw new IllegalArgumentException("There is already a station registered with name " + name);
  ...
  public DockStation getDefaultStation() {
    if (defaultStation != null) return defaultStation;

    Iterator<RootInfo> infos = roots.values().iterator();
    if (infos.hasNext()) return infos.next().getStation();
  ...
  public void removeRoot(DockStation station) {
    ...
    if (info != null) {
      if (defaultStation == info.getStation()) defaultStation = null;
  ...
  public boolean hasLocation(Dockable dockable) {
    ...
    if (isShown(dockable)) return true;
  ...
  public void setCurrentSetting(String setting) {
    if (setting == null) throw new IllegalArgumentException("the name of a setting must not be null");
  ...
  public boolean isHideable(Dockable dockable) {
    DockInfo info = getInfo(dockable);
    if (info == null) throw new IllegalArgumentException("Dockable not registered");
  ...
  public void setEntryLayout(String id, boolean layout) {
    DockInfo info = getInfo(id);
    if (info == null) throw new IllegalArgumentException("no entry present for: " + id);
      ...
      if (isHidden(dockable) || isHiddenRootStation(dockable)) {
        if (veto.expectToShow(dockable, cancelable)) {
          ...
          if (info == null) {
            ...
            if (station == null) throw new IllegalStateException("Can't find the default station");
            ...
            if (location == null) {
              ...
              if (!station.drop(dockable, location)) getDefaultStation().drop(dockable);
            }
          }
   ...
   */
  public void save() {
    if (currentSetting == null) throw new IllegalStateException("No setting loaded yet");
  ...
  private void fillMissing() {
    ...
    if (lastAppliedEntrySetting != null &&
        lastAppliedEntrySetting != lastAppliedFullSetting &&
      ...
      if (info.getDataLayout().getFactoryID().equals(factoryId)) {
        ...
        if (location != null) {
          ...
          if (missingDockable.shouldCreate(normalizedFactory, info.getDataLayout().getData())) {
            DockElement element =
              normalizedFactory.layout(info.getDataLayout().getData(), layoutChangeStrategy.getPlaceholderStrategy(new Internals()));
            if (element != null) {
              Dockable dockable = element.asDockable();
              if (dockable != null) {
      ...
      if (!ignore.ignoreChildren(station)) {
        for (int i = station.getDockableCount() - 1; i >= 0; i--) {
          ...
          if (!ignore.ignoreElement(dockable)) {
            ...
            if (check != null) clean(check, ignore);
  ...
  public boolean delete(String name) {
    ...
    if (deleted) {
      if (name.equals(currentSetting)) currentSetting = null;
  ...
  public SettingsBlop readBlop(DataInputStream in) throws IOException {
    ...
    if (in.readBoolean()) currentSetting = in.readUTF();
  ...
  public void writeBlopXML(SettingsBlop blop, XElement element) {
    ...
    if (current != null) xcurrent.addString("name", current);
  ...
  private DockInfo getInfo(Dockable dockable) {
    ...
    for (DockInfo info : dockables.values()) {
      if (info.getDockable() == dockable) return info;
    }
  ...
  private RootInfo getRoot(DockStation station) {
    for (RootInfo info : roots.values()) {
      if (info.getStation() == station) return info;
    }
