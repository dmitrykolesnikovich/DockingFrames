...
public class DockController {
   ...
   */
  public static final PropertyKey<Boolean> RESTRICTED_ENVIRONMENT = new PropertyKey<Boolean>("dock.restricted_environment",
                                                                                             new ConstantPropertyFactory<Boolean>(
                                                                                               DockUI.getDefaultDockUI()
  ...
  protected final void initiate(DockControllerFactory factory, ControllerSetupCollection setup) {
    if (this.factory != null) throw new IllegalStateException("DockController already initialized");
    ...
    if (focus != null) register.addDockRegisterListener(focus);
    ...
    if (binder != null) register.addDockRegisterListener(binder);
    ...
    properties
      .set(DockableSelector.INIT_SELECTION, KeyStroke.getKeyStroke(KeyEvent.VK_E, KeyEvent.CTRL_DOWN_MASK | KeyEvent.SHIFT_DOWN_MASK));
  ...
  protected UIScheme<String, TextValue, TextBridge> createDefaultTextScheme() {
    ResourceBundle bundle =
      ResourceBundle.getBundle("data.bibliothek.gui.dock.core.locale.text", Locale.getDefault(), DockController.class.getClassLoader());
  ...
  public ComponentHierarchyObserver getComponentHierarchyObserver() {
    if (componentHierarchyObserver == null) {
      ...
      if (rootWindow != null) componentHierarchyObserver.add(rootWindow);
    }
  ...
  public boolean meltLayout() {
    if (!isLayoutFrozen()) throw new IllegalStateException("the layout is not frozen");
  ...
  public void setDefaultActionOffer(ActionOffer defaultActionOffer) {
    if (defaultActionOffer == null) throw new IllegalArgumentException();
  ...
  public ActionOffer getActionOffer(Dockable dockable) {
    for (ActionOffer offer : actionOffers) {
      if (offer.interested(dockable)) return offer;
    }
  ...
  public void addRepresentative(DockElementRepresentative representative) {
    DockControllerRepresentativeListener[] listeners =
      componentToDockElementsListeners.toArray(new DockControllerRepresentativeListener[componentToDockElementsListeners.size()]);
  ...
  public DockElementRepresentative searchElement(Component representative) {
    while (representative != null) {
      ...
      if (element != null) {
        if (element.getElement().getController() == this) return element;
      }
   ...
   */
  @Deprecated
  @Todo(compatibility = Compatibility.BREAK_MAJOR, description = "remove this method", priority = Todo.Priority.ENHANCEMENT, target = Version.VERSION_1_1_3)
  ...
  protected void rootWindowChanged(Window oldWindow, Window newWindow) {
    if (componentHierarchyObserver != null) {
      if (oldWindow != null) componentHierarchyObserver.remove(oldWindow);

      if (newWindow != null) componentHierarchyObserver.add(newWindow);
    }
  }
  ...
  public Window findRootWindow() {
    if (rootWindow != null) return rootWindow;
  ...
  public void addActionGuard(ActionGuard guard) {
    if (guard == null) throw new IllegalArgumentException("guard must not be null");
  ...
  public DockActionSource listOffers(Dockable dockable) {
    ...
    for (ActionGuard guard : this.guards) {
      if (guard.react(dockable)) guards.add(guard.getSource(dockable));
    }

    ActionOffer offer = getActionOffer(dockable);
    DockActionSource parentSource = null;

    if (dockable.getDockParent() != null) parentSource = dockable.getDockParent().getDirectActionOffers(dockable);

    return offer.getSource(dockable, dockable.getLocalActionOffers(), guards.toArray(new DockActionSource[guards.size()]), parentSource,
                           parents.toArray(new DockActionSource[parents.size()]));
  }
  ...
  private class DockTitleObserver extends DockRegisterAdapter implements DockTitleBindingListener {
    ...
    private void handleAddedTitle(Dockable dockable, DockTitle title) {
      ...
      if (focused) activeTitles.put(title, dockable);
    }
