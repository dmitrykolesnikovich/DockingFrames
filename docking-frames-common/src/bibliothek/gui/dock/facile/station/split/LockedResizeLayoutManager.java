...
public abstract class LockedResizeLayoutManager<T> extends DelegatingSplitLayoutManager {
  ...
  public void setConflictResolver(ConflictResolver<T> conflictResolver) {
    if (conflictResolver == null) throw new IllegalArgumentException("conflictResolver must not be null");
  ...
  public ResizeElement<T> toElement(ResizeElement<T> parent, SplitNode node) {
    ...
    System.err.println(
      "LockedResizeLayoutManager.toElement: Potential race condition detected, converting SplitNode to ResizeElement failed 5 times in a row. The node is ignored.");
    return null;
  }
