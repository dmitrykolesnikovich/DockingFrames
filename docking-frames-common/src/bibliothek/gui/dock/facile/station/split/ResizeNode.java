...
public class ResizeNode<T> extends ResizeElement<T> {
   ...
   */
  @SuppressWarnings("unchecked") private ResizeElement<T>[] children = new ResizeElement[2];
  ...
  public void adapt(double deltaWidth, double deltaHeight) {
    ...
    if (horizontal) {
      if (alpha != null && alpha.getFractionWidth() == -1) alpha = null;
      if (beta != null && beta.getFractionWidth() == -1) beta = null;
    }
    else {
      if (alpha != null && alpha.getFractionHeight() == -1) alpha = null;
      if (beta != null && beta.getFractionHeight() == -1) beta = null;
    }
