...
public class DefaultConflictResolver<T> implements ConflictResolver<T> {
  ...
  public ResizeRequest requestHorizontal(ResizeRequest left, ResizeRequest right, ResizeNode<T> node) {
    ...
    if (left == null) {
      return new ResizeRequest(right.getDeltaWidth(), right.getDeltaHeight(), increment(right.getFractionWidth()),
                               right.getFractionHeight());
    }

    if (right == null) {
      return new ResizeRequest(left.getDeltaWidth(), left.getDeltaHeight(), increment(left.getFractionWidth()), left.getFractionHeight());
    }
    ...
    if (left.getFractionWidth() == -1 && right.getFractionWidth() == -1) {
      ...
      widthDelta =
        left.getDeltaWidth() / left.getFractionWidth() + right.getDeltaWidth() / right.getFractionWidth() + node.getNewDividerSize() -
        node.getOldDividerSize();
      widthFraction = 1;
    }
    ...
    if (widthFraction == -1 && heightFraction == -1) return null;
  ...
  public ResizeRequest requestVertical(ResizeRequest top, ResizeRequest bottom, ResizeNode<T> node) {
    ...
    if (top == null) {
      return new ResizeRequest(bottom.getDeltaWidth(), bottom.getDeltaHeight(), bottom.getFractionWidth(),
                               increment(bottom.getFractionHeight()));
    }

    if (bottom == null) {
      return new ResizeRequest(top.getDeltaWidth(), top.getDeltaHeight(), top.getFractionWidth(), increment(top.getFractionHeight()));
    }
    ...
    if (top.getFractionHeight() == -1 && bottom.getFractionHeight() == -1) {
      ...
      heightDelta =
        top.getDeltaHeight() / top.getFractionHeight() + bottom.getDeltaHeight() / bottom.getFractionHeight() + node.getNewDividerSize() -
        node.getOldDividerSize();
      heightFraction = 1;
    }
