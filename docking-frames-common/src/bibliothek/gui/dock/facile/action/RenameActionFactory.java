...
public class RenameActionFactory {
  ...
  public RenameAction find(Dockable owner) {
    ...
    if (action == null) throw new IllegalStateException("Can't find a rule for " + clazz.getName());
    return action;
  }
  ...
  protected RenameAction create(Class<?> owner) {
    if (owner.equals(DefaultDockable.class)) return new RenameAction.RenameDefaultDockable(controller);

    if (owner.equals(SplitDockStation.class)) return new RenameAction.RenameSplitDockStation(controller);

    if (owner.equals(FlapDockStation.class)) return new RenameAction.RenameFlapDockStation(controller);

    if (owner.equals(StackDockStation.class)) return new RenameAction.RenameStackDockStation(controller);
