...
public class KeyedActionSource extends AbstractDockActionSource {
  ...
  public void setVisible(boolean visible) {
    if (this.visible != visible) {
      ...
      else if (current > updated) fireRemoved(0, 0);
    }
  }
  ...
  public DockAction getDockAction(int index) {
    if (index != 0) throw new IllegalArgumentException("only index=0 supported");
  ...
  public Iterator<DockAction> iterator() {
    return new Iterator<DockAction>() {
      ...
      public DockAction next() {
        if (action == null) throw new IllegalStateException("no elements left");
