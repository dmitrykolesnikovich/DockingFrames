...
public class SeparatingMenuPiece extends MenuPiece {
  ...
  public void fill(List<Component> items) {
    if (piece == null || piece.getItemCount() == 0) {
      if (emptySeparator) items.add(getEmptySeparator());
    }
    else {
      if (topSeparator) items.add(getTopSeparator());

      piece.fill(items);

      if (bottomSeparator) items.add(getBottomSeparator());
    }
  }
  ...
  protected int getSeparatorCount() {
    if (piece != null && piece.getItemCount() > 0) {
      if (topSeparator && bottomSeparator) return 2;

      if (topSeparator || bottomSeparator) return 1;

      return 0;
    }
    else {
      if (emptySeparator) return 1;
  ...
  private void putUpSeparators() {
    ...
    else if (!top && separatorTop != null) separatorTop = null;

    if (bottom && separatorBottom == null) {
      separatorBottom = new JPopupMenu.Separator();
    }
    else if (!top && separatorBottom != null) separatorBottom = null;
  }
  ...
  private class Listener implements MenuPieceListener {
    public void insert(MenuPiece child, int index, Component... component) {
      if (component.length > 0) {
        ...
        if (count == 0) {
          if (emptySeparator) fireRemove(0, 1);

          if (bottomSeparator) fireInsert(0, getBottomSeparator());

          if (topSeparator) fireInsert(0, getTopSeparator());
        }

        if (topSeparator) index++;
    ...
    public void remove(MenuPiece child, int index, int length) {
      if (length > 0) {
        ...
        if (child.getItemCount() == 0) {
          ...
          if (emptySeparator) fireInsert(0, getEmptySeparator());
        }
      }
    }
  }
