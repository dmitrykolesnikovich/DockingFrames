...
public abstract class ModeManager<H, M extends Mode<H>> {
  ...
  private ModeHandle getAccess(Path path) {
    for (ModeHandle mode : modes) {
      if (mode.mode.getUniqueIdentifier().equals(path)) return mode;
    }
    return null;
  }
  ...
  public void apply(Dockable dockable, M mode, AffectedSet set, boolean force) {
    ...
    if (entry == null) throw new IllegalArgumentException("dockable not registered");

    M dockableMode = getCurrentMode(dockable);
    if (!force && dockableMode == mode) return;
  ...
  public void addEmpty(String key) {
    if (key == null) throw new NullPointerException("name must not be null");
  ...
  protected M getDefaultMode(Dockable dockable) {
    if (modes.isEmpty()) throw new IllegalStateException("no modes available");
  ...
  private class DockableHandle {
    ...
    public void updateActionSource() {
      if (dockable != null) {
        ...
        if (mode == null) mode = getDefaultMode(dockable);
