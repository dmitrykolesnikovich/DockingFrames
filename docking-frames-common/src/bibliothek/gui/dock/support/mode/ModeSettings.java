...
public class ModeSettings<A, B> {
  ...
  public void add(Mode<A> mode) {
    ...
    if (factory == null) throw new IllegalArgumentException("no factory present for '" + mode.getUniqueIdentifier() + "'");
  ...
  public void writeXML(XElement element) {
    ...
    for (DockableEntry entry : dockables) {
      ...
      if (entry.current != null) xentry.addString("current", entry.current.toString());
  ...
  public void readXML(XElement element) {
    ...
    if (delement != null) {
      for (XElement xentry : delement.getElements("entry")) {
        ...
        if (current != null) entry.current = new Path(current.getString());
