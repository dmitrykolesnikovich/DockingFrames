...
public class BubbleDisplayerTransmitter extends ColorTransmitter {
  private static final String[] KEYS =
    {"displayer.border.high.active", "displayer.border.high.active.mouse", "displayer.border.high.inactive",
      "displayer.border.high.inactive.mouse", "displayer.border.low.active", "displayer.border.low.active.mouse",
      "displayer.border.low.inactive", "displayer.border.low.inactive.mouse"};
  ...
  protected Color get(Color color, String id, CDockable dockable) {
    ...
    if (id.contains("active") && !id.contains("inactive")) {
      ...
      if (check != null) check = convertFocused(check, id);
    }

    if (check == null) {
      check = map.getColor(ColorMap.COLOR_KEY_TITLE_BACKGROUND);
      if (check != null) check = convert(check, id);
    }
  ...
  protected void update(CDockable dockable, String key, Color color) {
    boolean change = ColorMap.COLOR_KEY_TITLE_BACKGROUND.equals(key) || ColorMap.COLOR_KEY_TITLE_BACKGROUND_FOCUSED.equals(key);
  ...
  protected Color convert(Color source, String key) {
    if (key.contains("active") && !key.contains("inactive")) return convertFocused(Colors.diffMirror(source, 0.2), key);

    if ("displayer.border.low.inactive".equals(key)) return Colors.darker(source, 0.3);
    if ("displayer.border.high.inactive".equals(key)) return Colors.brighter(source, 0.3);

    if ("displayer.border.low.inactive.mouse".equals(key)) return Colors.fuller(Colors.darker(source, 0.3), 0.3);
    if ("displayer.border.high.inactive.mouse".equals(key)) return Colors.fuller(Colors.brighter(source, 0.3), 0.3);
  ...
  protected Color convertFocused(Color source, String key) {
    if ("displayer.border.low.active".equals(key)) return Colors.darker(source, 0.3);
    if ("displayer.border.high.active".equals(key)) return Colors.brighter(source, 0.3);

    if ("displayer.border.low.active.mouse".equals(key)) return Colors.fuller(Colors.darker(source, 0.3), 0.3);
    if ("displayer.border.high.active.mouse".equals(key)) return Colors.fuller(Colors.brighter(source, 0.3), 0.3);
  ...
  protected CDockable getDockable(DockColor observer) {
    ...
    if (dockable instanceof CommonDockable) return ((CommonDockable)dockable).getDockable();
