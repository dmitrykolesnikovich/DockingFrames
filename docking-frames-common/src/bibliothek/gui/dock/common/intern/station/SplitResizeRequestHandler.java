  ...
  private class LayoutManager extends CLockedResizeLayoutManager {
    ...
    public RequestDimension prepareResize(Leaf leaf) {
      ...
      if (request != null) {
        ...
        if (request.isWidthSet()) request.setWidth(request.getWidth() + insets.left + insets.right);

        if (request.isHeightSet()) request.setHeight(request.getHeight() + insets.top + insets.bottom);
