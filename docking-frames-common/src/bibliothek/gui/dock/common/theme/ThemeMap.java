...
public class ThemeMap {
  ...
  public ThemeMap(final CControl control) {
    ...
    addThemeMapListener(new ThemeMapListener() {
      ...
      public void selectionChanged(ThemeMap map, String oldKey, String newKey) {
        ...
        if (newKey != null) factory = getFactory(newKey);
        ...
        if (modifier != null) theme = modifier.modify(theme);
  ...
  private void init(CControl control) {
    ThemeFactory flat = new CDockThemeFactory<FlatTheme>(new ThemePropertyFactory<FlatTheme>(FlatTheme.class), control) {
      @Override
    ...
    ThemeFactory bubble = new CDockThemeFactory<BubbleTheme>(new ThemePropertyFactory<BubbleTheme>(BubbleTheme.class), control) {
      @Override
      public DockTheme create(CControl control) {
    ...
    ThemeFactory smooth = new CDockThemeFactory<SmoothTheme>(new ThemePropertyFactory<SmoothTheme>(SmoothTheme.class), control) {
      @Override
      public DockTheme create(CControl control) {
    ...
    ThemeFactory basic = new CDockThemeFactory<BasicTheme>(new ThemePropertyFactory<BasicTheme>(BasicTheme.class), control) {
      @Override
      public DockTheme create(CControl control) {
  ...
  public void select(ThemeFactory factory) {
    ...
    if (index < 0) throw new IllegalArgumentException("factory not known " + factory);
  ...
  public void select(int index, boolean force) {
    Entry entry = null;
    if (index >= 0) entry = factories.get(index);
  ...
  public void insert(int index, String key, ThemeFactory factory) {
    ...
    if (index < 0 || index > factories.size()) throw new ArrayIndexOutOfBoundsException(index);
  ...
  public int indexOf(ThemeFactory factory) {
    ...
    for (Entry entry : factories) {
      if (entry.factory == factory) return index;
  ...
  public int indexOf(String key) {
    int index = 0;
    for (Entry entry : factories) {
      if (entry.key.equals(key)) return index;
