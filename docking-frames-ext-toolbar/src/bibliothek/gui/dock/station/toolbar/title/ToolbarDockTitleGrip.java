...
public class ToolbarDockTitleGrip extends AbstractDockTitle {
  ...
  public static DockTitleFactory createFactory(final Color backgroundColor, final Color gripColor, final Color gripShadowColor) {
    return new DockTitleFactory() {
      ...
      public void request(DockTitleRequest request) {
        request.answer(new ToolbarDockTitleGrip(request.getVersion(), request.getTarget(), backgroundColor, gripColor, gripShadowColor));
      }
  ...
  public static DockTitleFactory createFactory() {
    return new DockTitleFactory() {
      ...
      @Override
      public void request(DockTitleRequest request) {
        request.answer(new ToolbarDockTitleGrip(request.getVersion(), request.getTarget()));
      }
  ...
  protected void paintComponent(Graphics g) {
    ...
    if (getOrientation().isHorizontal()) {
      ...
      g.drawLine(lineOffset, headerOffset, width - lineOffset, headerOffset);
      g.drawLine(lineOffset, headerOffset + 1, width - lineOffset, headerOffset + 1);

      // Draw the shadow.
      g.setColor(gripShadowColor);
      g.drawLine((width - lineOffset) + 1, headerOffset, (width - lineOffset) + 1, headerOffset + 2);
      g.drawLine(lineOffset, headerOffset + 2, width - lineOffset, headerOffset + 2);
      ...
      // Draw the light line.
      g.setColor(gripColor);
      g.drawLine(headerOffset, lineOffset, headerOffset, height - lineOffset);
      g.drawLine(headerOffset + 1, lineOffset, headerOffset + 1, height - lineOffset);

      // Draw the shadow.
      g.setColor(gripShadowColor);
      g.drawLine(headerOffset, (height - lineOffset) + 1, headerOffset + 2, (height - lineOffset) + 1);
      g.drawLine(headerOffset + 2, lineOffset, headerOffset + 2, height - lineOffset);
    }
  }
