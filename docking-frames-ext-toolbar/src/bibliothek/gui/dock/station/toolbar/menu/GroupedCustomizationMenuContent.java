...
public class GroupedCustomizationMenuContent implements CustomizationMenuContent {
  ...
  public class Group {
    ...
    private int insertItems(int offset) {
      int begin = offset;
      if (title != null) {
        view.add(title.getView(),
                 new GridBagConstraints(0, offset, columns, 1, 1.0, 0.01, GridBagConstraints.CENTER, GridBagConstraints.HORIZONTAL, insets,
                                        0, 0));
        offset++;
      }
      int index = 0;
      ...
      while (index < length) {
        for (int i = 0; i < columns && index < length; i++) {
          view.add(items.get(index++).getView(),
                   new GridBagConstraints(i, offset, 1, 1, 1.0, 1.0, GridBagConstraints.CENTER, GridBagConstraints.BOTH, insets, 0, 0));
        }
        offset++;
      }
      return offset - begin;
