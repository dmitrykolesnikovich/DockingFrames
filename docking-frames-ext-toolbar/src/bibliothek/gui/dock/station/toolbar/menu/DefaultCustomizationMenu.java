 ...
 */
@Todo(compatibility = Compatibility.COMPATIBLE, target = Version.VERSION_1_1_2, priority = Priority.MINOR, description = "This class is almost identical with 'DialogWindow' from Common, maybe these two classes can be merged?")
public class DefaultCustomizationMenu extends AbstractCustomizationMenu {
  ...
  protected JDialog createDialog(Component owner) {
    ...
    if (window instanceof Frame) return new JDialog((Frame)window);
    if (window instanceof Dialog) return new JDialog((Dialog)window);
    return new JDialog();
  }
}
