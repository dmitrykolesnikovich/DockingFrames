...
public abstract class ToolbarContainerDropInfo implements StationDropOperation {
    ...
    switch (stationHost.getOrientation()) {
      case VERTICAL:
        ...
        final double middleY =
          (dockableBeneathMouse.getComponent().getBounds().getMinY() + dockableBeneathMouse.getComponent().getBounds().getMaxY()) / 2.0;
      ...
      case HORIZONTAL:
        ...
        final double middleX =
          (dockableBeneathMouse.getComponent().getBounds().getMinX() + dockableBeneathMouse.getComponent().getBounds().getMaxX()) / 2.0;
  ...
  private Dockable computeDockableBeneathMouse() {
    final DockablePlaceholderList.Filter<StationChildHandle> associateToolbars = this.associateToolbars.dockables();
    ...
    switch (orientation) {
      case VERTICAL:
        componentBeneathMouse = associateToolbars.get(0).getDisplayer().getComponent();
        ...
        for (int i = 1; i < dockableCount; i++) {
          componentBeneathMouse = associateToolbars.get(i).getDisplayer().getComponent();
