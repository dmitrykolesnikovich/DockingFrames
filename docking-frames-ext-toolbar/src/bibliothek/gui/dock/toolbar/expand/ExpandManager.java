...
public class ExpandManager {
  ...
  protected void update(ScreenDockStation station, Dockable child) {
    ...
    final MagnetizedOperation operation = station.getMagnetController().start(window);

    final Rectangle validated = station.getBoundaryRestriction().check(window, bounds);
    if (validated != null) {
      bounds = validated;
    }
