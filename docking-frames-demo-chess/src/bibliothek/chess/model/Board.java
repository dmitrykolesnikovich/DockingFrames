...
public class Board {
  ...
  public Figure pawnReplacement() {
    for (int c = 0; c < 8; c++) {
      if (isType(0, c, Figure.Type.PAWN)) return getFigure(0, c);

      if (isType(7, c, Figure.Type.PAWN)) return getFigure(7, c);
    }
  ...
  public void move(int sr, int sc, int dr, int dc) {
    ...
    figures[sr][sc] = null;
    if (figures[dr][dc] != null) kill(dr, dc);
      ...
      for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
          if (i != dr || j != dc) {
            ...
            if (check != null) check.cleanJustMoved();
          }
        }
      }
    }
