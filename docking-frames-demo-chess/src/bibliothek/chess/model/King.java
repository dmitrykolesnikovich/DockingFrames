...
public class King extends Behavior {
  ...
  private void reachable(Board board, Figure figure, CellVisitor visitor, boolean ignoreAttack) {
    if (!ignoreAttack) visitor = createAttackVisitor(board, figure, visitor);
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (board.isValid(r, c)) {
        if (ignoreAttack) {
          if (!board.visit(r, c, visitor)) return;
        }
        else if (board.isEmpty(r, c) || board.isPlayer(r, c, figure.getPlayer().opponent())) {
          if (!board.isAttacked(r, c)) if (!board.visit(r, c, visitor)) return;
        }
      }
    }
    ...
    if (!figure.isMoved()) {
      ...
      if (board.isEmpty(r, c + 1) && !board.isAttacked(r, c + 1) && board.isEmpty(r, c + 2) && !board.isAttacked(r, c + 2)) {
        Figure rock = board.getFigure(r, c + 3);
        if (rock != null && rock.getType() == Figure.Type.ROCK) {
          if (rock.getPlayer() == figure.getPlayer() && !rock.isMoved()) if (!board.visit(r, c + 2, visitor)) return;
        }
      }

      if (board.isEmpty(r, c - 1) &&
          !board.isAttacked(r, c - 1) &&
          board.isEmpty(r, c - 2) &&
          !board.isAttacked(r, c - 2) &&
          board.isEmpty(r, c - 3)) {
