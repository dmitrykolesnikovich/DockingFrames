...
public class Pawn extends Behavior {
  ...
  public void attackable(Board board, Figure figure, CellVisitor visitor) {
    ...
    if (board.isValid(r, c + 1)) if (!board.visit(r, c + 1, visitor)) return;
  ...
  public void reachable(Board board, Figure figure, CellVisitor visitor) {
    ...
    if (board.isValid(r, c) && board.isEmpty(r, c)) {
      ...
      if (!figure.isMoved()) {
        r += plus;
        if (board.isValid(r, c) && board.isEmpty(r, c)) if (!board.visit(r, c, visitor)) return;
      }
    }

    // catch
    r = figure.getRow() + plus;
    if (board.isValid(r, c + 1) && board.isPlayer(r, c + 1, figure.getPlayer().opponent())) if (!board.visit(r, c + 1, visitor)) return;
    if (board.isValid(r, c - 1) && board.isPlayer(r, c - 1, figure.getPlayer().opponent())) if (!board.visit(r, c - 1, visitor)) return;
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (board.isValid(r, c)) {
          ...
          if (pawn != null && pawn.getType() == Figure.Type.PAWN) {
            if (pawn.isJustMoved() && pawn.getPlayer().opponent() == figure.getPlayer()) if (!board.visit(r + plus, c, visitor)) return;
          }
        }
      }
    }
