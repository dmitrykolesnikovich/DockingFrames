...
public class ChessBoard extends OverpaintablePanel implements DockStation, ChessListener {
  ...
  public void setBoard(Board board) {
    if (this.board != null) this.board.removeListener(this);
  ...
  private Field getFieldOf(Dockable figure) {
    for (Field field : usedFieldList) {
      if (field.getFigure() == figure) return field;
    }
  ...
  private StationDropOperation prepare(int x, int y, ChessFigure figure) {
    ...
    while (c * w / 8 > location.x) c--;

    while ((7 - r) * h / 8 > location.y) r++;
  ...
  private class Field {
    ...
    public void set(ChessFigure figure) {
      ...
      if (this.figure != null) {
        ...
        if (controller != null) version = controller.getDockTitleManager().getVersion("chess-board");
      ...
      else if (old != null && figure == null) usedFieldList.remove(this);
