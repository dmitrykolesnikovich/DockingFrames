...
public class IconGrid extends JPanel {
  ...
  protected void paintComponent(Graphics g) {
    ...
    for (int y = 0; y < sqrt; y++) {
      for (int x = 0; x < sqrt; x++) {
        if (index >= ResourceSet.NOTE_ICONS.size()) break loop;
        ...
        icon.paintIcon(this, g, x * GRID_SIZE + (GRID_SIZE - ICON_SIZE) / 2, y * GRID_SIZE + (GRID_SIZE - ICON_SIZE) / 2);
  ...
  private int iconAt(int x, int y) {
    ...
    int index = x + y * sqrt;
    if (index < 0 || index > ResourceSet.NOTE_ICONS.size()) return -1;
