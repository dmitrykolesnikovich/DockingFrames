...
public class StorageDockable extends DefaultSingleCDockable {
  ...
  private class Entry extends AbstractPreference<Object> {
    ...
    public PreferenceOperation[] getOperations() {
      return new PreferenceOperation[]{PreferenceOperation.DELETE, LOAD, STORE_BINARY, STORE_XML};
    }

    @Override
    public boolean isEnabled(PreferenceOperation operation) {
      if (operation == STORE_XML || operation == STORE_BINARY) return true;

      if (operation == LOAD || operation == PreferenceOperation.DELETE) return data != null;
    ...
    public void doOperation(PreferenceOperation operation) {
      try {
        ...
        ex.printStackTrace();
        JOptionPane
          .showMessageDialog(getContentPane(), "Unable to perform operation:\n" + ex.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
      }
    }
