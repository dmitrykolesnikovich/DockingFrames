...
public class ModifyMultiDockable extends DefaultSingleCDockable {
  private static final PreferenceOperation ADD_MULTI_FACTORY =
    new PreferenceOperation("add factory", Icons.get("add factory"), "CControl.add( String, MultipleCDockableFactory");
  private static final PreferenceOperation REMOVE_MULTI_FACTORY =
    new PreferenceOperation("remove factory", Icons.get("remove factory"), "CControl.removeSingleBackupFactory");
  ...
  private class Entry extends AbstractPreference<String> {
    ...
    public PreferenceOperation[] getOperations() {
      return new PreferenceOperation[]{PreferenceOperation.DELETE, ADD_MULTI_DOCKABLE, REMOVE_MULTI_FACTORY, ADD_MULTI_FACTORY};
    }

    @Override
    public boolean isEnabled(PreferenceOperation operation) {
      if (operation == PreferenceOperation.DELETE) return true;

      boolean present = core.getEnvironment().getEnvironmentControl().getMultipleDockableFactory(id) != null;

      if (operation == ADD_MULTI_DOCKABLE) return present;
      if (operation == ADD_MULTI_FACTORY) return !present;
      if (operation == REMOVE_MULTI_FACTORY) return present;
      return false;
    }
  ...
  private class NewEntry extends AbstractPreference<String> {
    ...
    public boolean isEnabled(PreferenceOperation operation) {
      ...
      if (allFactories.containsKey(value)) return false;

      if (!Path.isValidPath(value)) return false;
